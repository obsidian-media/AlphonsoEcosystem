...
_Last verified: 2026-07-02 — v2.5.1 — ALPHONSOTOTHEMOON Sprint 1 shipped: SHALAUDE License v1.0 added (`LICENSE` — the repo previously had no license file; README's stale "BSL 1.1" badge/section corrected to match), `ALPHONSOTOTHEMOON.md` roadmap added (comparison against OpenFang/LibreFang orchestration patterns, sprint tracking), `validateSkillPackAgainstContract()` added to `agentContractService.ts` and wired into `skillPackService.js` install/enable paths, default skill packs added for the 5 agents that lacked one (Alphonso, Marcus, Echo, Sentinel, Nova — all 9 agents now covered), loop-guard/execution budget (`PIPELINE_MAX_ASSIGNMENTS`=50, `PIPELINE_MAX_DURATION_MS`=5min) added to `runJoseCommandExecutionPipeline`. TypeScript: 0 errors. Targeted Sprint 1 tests: 105/105 passing (`skillPackService.test.js`, `joseExecutionEngineService.test.js`). Full 218-file suite could not complete in one run on this dev machine — vitest worker-pool startup times out past ~170 files regardless of pool size (reproduced with default settings, a 4-worker cap, and the project's own `scripts/run-vitest-programmatic.mjs`); every file that did run passed with 0 assertion failures — logged as an open environment item under Real Gaps, not treated as a code defect. `docs/CHANGELOG.md`, `README.md`, and `docs/ALPHONSO_GROUND_TRUTH.md` updated same-pass. package.json version: 2.5.1. Previous entry (v2.5.0, 2026-07-02): Security hardening (Batch 1) complete — SSRF blocking, PKCE OAuth, tauri-plugin-dialog, arboard clipboard, per-program arg allowlist, policyDslService wired, CSP narrowed; 218 files / 3,174+ tests passing; Rust 91 passed / 3 pre-existing failures unchanged; onboarding wizard expanded to 6 steps; 8 Dependabot PRs merged, 3 left open (rand 0.10, Tailwind v4, vite-plugin-react v6 — all breaking); companion backend 5 Rust modules live; 35+ new components and 40+ new services added since v2.0.5 — see Do Not Duplicate table above._
