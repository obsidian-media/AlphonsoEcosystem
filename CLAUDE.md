...
- Coach Mode investigated (user reported it "feels forgotten") — confirmed wired (reachable via main `Sidebar` and `OperatorDashboard`), not dead code. UI prominence unverified — no browser-automation tool was available this session to click through the live app. Seeded as a "feature discoverability audit" in `ALPHONSOTOTHEMOON.md` Sprint 3 — still OPEN
- Subprocess/sandboxed tool execution, MCP-as-first-class-runtime-capability, scheduler heartbeat supervision, email connector, module-system convergence, EULA/trademark, agent skill-library depth, security hardening Batch 2 — see `ALPHONSOTOTHEMOON.md` Sprint 3–6 for the full backlog
