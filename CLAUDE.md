...
- **ESLint does not lint any `.ts`/`.tsx` file in this repo.** Found 2026-07-02 while answering a user question about what's actually being verified: `eslint.config.js`'s only `files` block is `src/**/*.{js,jsx}` — there is no matching block for `.ts`/`.tsx` anywhere in the config. This means every "ESLint clean" claim in this document and in `ALPHONSOTOTHEMOON.md`, across every sprint, was accurate for the files ESLint actually processed but never covered the 114 `.tsx` components, the pre-existing `.ts` services (`agentContractService.ts`, `orchestrationQueueService.ts`, `discordConnector.ts`, etc.), or any of the 16 `.ts` files migrated in Sprint 5 batches 1-2. Fix: add a `files: ['src/**/*.{ts,tsx}']` block to `eslint.config.js` with an appropriate TS parser (`@typescript-eslint/parser`) and plugin — not done yet, needs its own pass since it may surface a backlog of findings across the whole TS surface at once. Still OPEN, tracked in `ALPHONSOTOTHEMOON.md`'s Sprint 5 tracker
