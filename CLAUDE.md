...
- ~~Stale claim: "10 components migrated, 63 .jsx remaining"~~ — **CORRECTED 2026-07-02**: components are actually 100% `.tsx` (114 files, 0 `.jsx`). Real remaining TS-migration gap is services (115 `.js` / 16 `.ts`) — tracked as Sprint 5 in `ALPHONSOTOTHEMOON.md`
- Coach Mode investigated (user reported it "feels forgotten") — confirmed wired (reachable via main `Sidebar` and `OperatorDashboard`), not dead code. UI prominence unverified — no browser-automation tool was available this session to click through the live app. Seeded as a "feature discoverability audit" in `ALPHONSOTOTHEMOON.md` Sprint 3 — still OPEN (this is the discoverability-audit half of Sprint 3, distinct from the skill-library-depth half closed below)
...
- Subprocess/sandboxed tool execution, MCP-as-first-class-runtime-capability, scheduler heartbeat supervision, email connector, module-system convergence, EULA/trademark, security hardening Batch 2, feature discoverability audit (Coach Mode etc.) — see `ALPHONSOTOTHEMOON.md` Sprint 3–6 for the full backlog
