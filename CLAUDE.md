...
_Last verified: 2026-07-02 — v2.5.0 — Security hardening (Batch 1) complete: SSRF blocking, PKCE OAuth, tauri-plugin-dialog, arboard clipboard, per-program arg allowlist, policyDslService wired, CSP narrowed (now also allows `http://127.0.0.1:8188` for the ComfyUI fetch path — see Real Gaps CLOSED entries). TypeScript: 0 errors. Tests: 218 files / 3,174+ passing (re-verified in isolation post-audit; +7 tests from onboarding wizard, +3 tests from `openExternalUrl`). Rust: 91 passed / 3 pre-existing failures (see `todo.md`, unchanged). Onboarding wizard expanded to 6 steps (approval-mode decision + optional-services check). Fixed a user-reported batch: connector credentials silently failing to persist across a KV/localStorage split-brain, ComfyUI falsely reported "not running" due to a CSP origin mismatch, Projects page had no submit affordance, Hector research citations were unclickable text due to `window.open()` no-oping in the Tauri webview, and iOS companion pairing failing because the mDNS advertisement's SRV host name was missing the required `.local.` suffix — see `docs/ALPHONSO_GROUND_TRUTH.md` "CLOSED — 2026-07-02" entries. Also corrected a stale ground-truth claim that plugin tools "run in the main thread with no isolation" — they already run as native subprocesses via `execute_plugin_tool`; closed the real gap (no per-plugin program scoping) via an optional `declared_programs` manifest allowlist. 8 Dependabot PRs merged (safe patches); 3 left open (rand 0.10, Tailwind v4, vite-plugin-react v6 — all breaking). Companion backend: 5 Rust modules live. 35+ new components and 40+ new services added since v2.0.5 — see Do Not Duplicate table above. package.json version: 2.5.0._
