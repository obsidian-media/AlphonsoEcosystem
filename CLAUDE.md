...
- **16 connectors** (`DEFAULT_CONNECTORS` in `connectorRegistry.js`): Telegram, WhatsApp, YouTube, mobile_bridge, ChatGPT, Claude, Qwen, Notion, ClickUp, SD WebUI, ComfyUI Video, Runway, GitHub, Slack, Discord, Generic Webhook — all policy-gated. All have credential input UI in ConnectorSetupPanel. (Brave Search, Ollama, Perplexity, Tavily, DeepSeek, n8n exist as separate connector services not yet folded into `DEFAULT_CONNECTORS` — see their individual entries in this table.)
...
- Subprocess/sandboxed tool execution, MCP-as-first-class-runtime-capability, scheduler heartbeat supervision, email connector, module-system convergence, EULA/trademark — see `ALPHONSOTOTHEMOON.md` §3–§5 for the full Sprint 3+ backlog, deliberately scoped out of Sprint 2
- `src/test/ConnectorSetupPanel.test.jsx` fails independent of any change in this session (7/7 tests) — `hydrateConnectorCredentialsFromSqlite` is not included in that file's `vi.mock('../services/connectors/connectorAuth', ...)` factory, so the real hydrate `useEffect` in `ConnectorSetupPanel.tsx` throws on mount. Reproduced identically with Sprint 2 changes stashed out, confirming it predates this session. Fix: add `hydrateConnectorCredentialsFromSqlite: vi.fn().mockResolvedValue()` to that file's mock factory — still OPEN
...
_Last verified: 2026-07-02 — v2.5.2 — ALPHONSOTOTHEMOON Sprint 2 shipped: crash-recovery checkpoint (`recoverInterruptedExecutions()` in `orchestrationQueueService.ts`, wired at boot in `App.tsx` — surfaces `markPacketInterrupted()`, which existed but was previously unwired), Discord connector (`discordConnector.ts`, Discord REST API v10, 17 tests), generic inbound webhook connector (`gateway/generic-webhook/` standalone deployable + `genericWebhookService.js` poller, 13 tests) — connector count now 16 (`DEFAULT_CONNECTORS.length`). TypeScript: 0 errors. Targeted Sprint 2 tests: 191/191 passing across all touched files (orchestrationQueueService, connectorGitHubSlack, connectorRegistryService, discordConnector, slackConnector, genericWebhookService, skillPackService, joseExecutionEngineService). Found and documented (not fixed) a pre-existing, session-independent failure in `ConnectorSetupPanel.test.jsx` (7/7 tests) — see Real Gaps. package.json version: 2.5.2. `docs/CHANGELOG.md`, `README.md`, and `docs/ALPHONSO_GROUND_TRUTH.md` updated same-pass (Sprint 1 initially missed this — corrected before Sprint 2 started; see `ALPHONSOTOTHEMOON.md` running log). Previous entry (v2.5.1, 2026-07-02): ALPHONSOTOTHEMOON Sprint 1 — SHALAUDE License v1.0 added, `ALPHONSOTOTHEMOON.md` roadmap added, `validateSkillPackAgainstContract()` added to `agentContractService.ts`, default skill packs added for all 9 agents, loop-guard/execution budget added to `runJoseCommandExecutionPipeline`. Full 218-file suite still cannot complete in one run on this dev machine — vitest worker-pool startup times out past ~170 files regardless of pool size; open environment item, not a code defect. Prior entry (v2.5.0, 2026-07-02): Security hardening (Batch 1) complete — SSRF blocking, PKCE OAuth, tauri-plugin-dialog, arboard clipboard, per-program arg allowlist, policyDslService wired, CSP narrowed; 218 files / 3,174+ tests passing; Rust 91 passed / 3 pre-existing failures unchanged; 35+ new components and 40+ new services added since v2.0.5 — see Do Not Duplicate table above._
