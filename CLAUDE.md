...
| Boardroom sessions | `src/components/BoardroomView.tsx` — multi-agent boardroom session model with Hector briefing (real RSS call), Jose routing (real command-bus call), a real Maria governance risk score on conclusion, and a functional (client-side only) risk-confirmation gate before distributing high-risk summaries; `src/components/BoardroomPanel.tsx` sidebar panel. Uses only the real 9-agent roster — `docs/BOARDROOM_MODEL_REGISTRY.md` and `docs/BOARDROOM_ROLES.md` are stale aspirational planning docs (11-seat design incl. "Hermes"/"Kairo"/"Shayan-as-seat", none of which exist in shipped code) — corrected with a banner 2026-07-10, kept for historical context only, do not treat as current. A large rebuild spec (bloome.im-style group chat, @mentions, agent critique/pushback, escalation, etc.) was proposed via a separate handoff PDF as of 2026-07-10 — **not started, not authorized** without explicit sign-off; do not build against it without confirming with the user first. |
...
- Auto-update full in-app download+install+relaunch — still OPEN, handed off via `docs/AUTO_UPDATE_HANDOFF.md` + PR #98 on `feat/in-app-auto-update` (needs `@tauri-apps/plugin-updater`/`plugin-process`, can only be properly verified against a real signed release)
...
- A large Boardroom rebuild spec (bloome.im-style group chat UX) was proposed via an external handoff PDF as of 2026-07-10 — explicitly **not started**, conflicts with a prior "don't touch Boardroom yet" instruction from the user; needs explicit sign-off before any Step 0 audit work begins on it — still OPEN pending user decision
