import { vi, describe, it, expect, beforeEach } from 'vitest';
import {
  getTelegramPollCursor,
  setTelegramPollCursor,
  browserPollTelegram,
  browserSendTelegram,
  parseTelegramCommand,
  handleTelegramBotCommand,
  verifyTelegramBotEnvironment
} from '../../services/telegramBrowserConnector';

beforeEach(() => {
  localStorage.clear();
  vi.restoreAllMocks();
  global.fetch = undefined;
});

describe('getTelegramPollCursor / setTelegramPollCursor', () => {
  it('returns null when no cursor has been set', () => {
    expect(getTelegramPollCursor()).toBeNull();
  });

  it('stores and retrieves a numeric cursor', () => {
    setTelegramPollCursor(42);
    expect(getTelegramPollCursor()).toBe(42);
  });

  it('returns null when storage contains invalid JSON', () => {
    localStorage.setItem('alphonso_telegram_poll_cursor_v1', 'bad-json');
    expect(getTelegramPollCursor()).toBeNull();
  });

  it('returns null when storage has non-numeric cursor', () => {
    localStorage.setItem('alphonso_telegram_poll_cursor_v1', JSON.stringify({ cursor: 'not-a-number' }));
    expect(getTelegramPollCursor()).toBeNull();
  });

  it('stores updatedAt timestamp alongside cursor', () => {
    setTelegramPollCursor(100);
    const raw = JSON.parse(localStorage.getItem('alphonso_telegram_poll_cursor_v1'));
    expect(raw.cursor).toBe(100);
    expect(typeof raw.updatedAt).toBe('number');
  });

  it('handles localStorage quota exceeded gracefully', () => {
    const spy = vi.spyOn(Storage.prototype, 'setItem').mockImplementation(() => {
      throw new Error('QuotaExceededError');
    });
    expect(() => setTelegramPollCursor(1)).not.toThrow();
    spy.mockRestore();
  });
});

describe('browserPollTelegram', () => {
  it('returns error when botToken is missing', async () => {
    const result = await browserPollTelegram({});
    expect(result.ok).toBe(false);
    expect(result.error).toBe('bot_token_missing');
    expect(result.count).toBe(0);
    expect(result.messages).toEqual([]);
  });

  it('returns error when botToken is empty string', async () => {
    const result = await browserPollTelegram({ botToken: '   ' });
    expect(result.ok).toBe(false);
    expect(result.error).toBe('bot_token_missing');
  });

  it('returns ok: false on fetch network error', async () => {
    global.fetch = vi.fn().mockRejectedValue(new Error('Network error'));
    const result = await browserPollTelegram({ botToken: 'tok123' });
    expect(result.ok).toBe(false);
    expect(result.error).toContain('Network error');
  });

  it('returns ok: false on non-200 HTTP status', async () => {
    global.fetch = vi.fn().mockResolvedValue({ ok: false, status: 401, text: async () => 'Unauthorized' });
    const result = await browserPollTelegram({ botToken: 'tok123' });
    expect(result.ok).toBe(false);
    expect(result.error).toContain('401');
  });

  it('returns ok: false when Telegram body.ok is false', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ ok: false, description: 'Unauthorized' })
    });
    const result = await browserPollTelegram({ botToken: 'tok123' });
    expect(result.ok).toBe(false);
    expect(result.error).toBe('Unauthorized');
  });

  it('uses default description when Telegram body.description is missing', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ ok: false })
    });
    const result = await browserPollTelegram({ botToken: 'tok123' });
    expect(result.error).toBe('telegram_get_updates_failed');
  });

  it('returns ok: true with parsed messages on success', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        ok: true,
        result: [
          {
            update_id: 100,
            message: {
              chat: { id: 1, type: 'private' },
              from: { id: 2, username: 'alice' },
              text: 'hello',
              date: 1700000000
            }
          }
        ]
      })
    });
    const result = await browserPollTelegram({ botToken: 'tok123' });
    expect(result.ok).toBe(true);
    expect(result.messages).toHaveLength(1);
    expect(result.messages[0].text).toBe('hello');
    expect(result.messages[0].chat_id).toBe('1');
    expect(result.messages[0].from_username).toBe('alice');
  });

  it('filters out messages without text or caption', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        ok: true,
        result: [
          {
            update_id: 100,
            message: {
              chat: { id: 1, type: 'private' },
              from: { id: 2 },
              text: '',
              date: 1
            }
          },
          {
            update_id: 101,
            message: {
              chat: { id: 1, type: 'private' },
              from: { id: 2 },
              date: 1
            }
          }
        ]
      })
    });
    const result = await browserPollTelegram({ botToken: 'tok123' });
    expect(result.messages).toHaveLength(0);
  });

  it('filters out updates without message or edited_message', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        ok: true,
        result: [
          { update_id: 100, callback_query: { data: 'btn' } }
        ]
      })
    });
    const result = await browserPollTelegram({ botToken: 'tok123' });
    expect(result.messages).toHaveLength(0);
  });

  it('handles edited_message updates', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        ok: true,
        result: [
          {
            update_id: 100,
            edited_message: {
              chat: { id: 1, type: 'group' },
              from: { id: 3 },
              text: 'edited text',
              date: 2
            }
          }
        ]
      })
    });
    const result = await browserPollTelegram({ botToken: 'tok123' });
    expect(result.messages).toHaveLength(1);
    expect(result.messages[0].text).toBe('edited text');
  });

  it('updates the poll cursor to the last update_id', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        ok: true,
        result: [{ update_id: 200, message: { chat: { id: 1, type: 'private' }, from: { id: 2 }, text: 'hi', date: 1 } }]
      })
    });
    await browserPollTelegram({ botToken: 'tok123' });
    expect(getTelegramPollCursor()).toBe(200);
  });

  it('sends offset parameter when cursor exists', async () => {
    setTelegramPollCursor(50);
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ ok: true, result: [] })
    });
    await browserPollTelegram({ botToken: 'tok123' });
    const url = global.fetch.mock.calls[0][0];
    expect(url).toContain('offset=51');
  });

  it('respects limit parameter', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ ok: true, result: [] })
    });
    await browserPollTelegram({ botToken: 'tok123', limit: 25 });
    const url = global.fetch.mock.calls[0][0];
    expect(url).toContain('limit=25');
  });

  it('uses caption as text fallback', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        ok: true,
        result: [
          {
            update_id: 100,
            message: {
              chat: { id: 1, type: 'private' },
              from: { id: 2 },
              caption: 'photo caption',
              date: 1
            }
          }
        ]
      })
    });
    const result = await browserPollTelegram({ botToken: 'tok123' });
    expect(result.messages[0].text).toBe('photo caption');
  });

  it('handles empty result array', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ ok: true, result: [] })
    });
    const result = await browserPollTelegram({ botToken: 'tok123' });
    expect(result.ok).toBe(true);
    expect(result.messages).toHaveLength(0);
    expect(result.count).toBe(0);
  });

  it('handles non-array result field', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ ok: true, result: 'not-an-array' })
    });
    const result = await browserPollTelegram({ botToken: 'tok123' });
    expect(result.messages).toEqual([]);
  });
});

describe('browserSendTelegram', () => {
  it('returns error when chatId is missing', async () => {
    const result = await browserSendTelegram({ botToken: 'tok', chatId: '', text: 'hello' });
    expect(result.ok).toBe(false);
    expect(result.error).toBe('chat_id_required');
  });

  it('returns error when text is missing', async () => {
    const result = await browserSendTelegram({ botToken: 'tok', chatId: '123', text: '' });
    expect(result.ok).toBe(false);
    expect(result.error).toBe('text_required');
  });

  it('returns error when botToken is missing', async () => {
    const result = await browserSendTelegram({ botToken: '', chatId: '123', text: 'hello' });
    expect(result.ok).toBe(false);
    expect(result.error).toBe('bot_token_missing');
  });

  it('returns ok: true with message metadata on success', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ ok: true, result: { message_id: 55 } })
    });
    const result = await browserSendTelegram({ botToken: 'tok', chatId: '123', text: 'Hello' });
    expect(result.ok).toBe(true);
    expect(result.external_id).toBe('55');
    expect(result.connector_id).toBe('telegram');
    expect(result.trust).toBe('verified');
  });

  it('returns null external_id when message_id is not a number', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ ok: true, result: { message_id: 'str-id' } })
    });
    const result = await browserSendTelegram({ botToken: 'tok', chatId: '123', text: 'Hello' });
    expect(result.external_id).toBeNull();
  });

  it('does not retry on 400 Bad Request', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: false,
      status: 400,
      json: async () => ({ ok: false, description: 'Bad Request: invalid chat_id' })
    });
    const result = await browserSendTelegram({ botToken: 'tok', chatId: '123', text: 'Hello' });
    expect(global.fetch).toHaveBeenCalledTimes(1);
    expect(result.ok).toBe(false);
    expect(result.error).toContain('Bad Request');
  });

  it('does not retry on 401 Unauthorized', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: false,
      status: 401,
      json: async () => ({ ok: false, description: 'Unauthorized' })
    });
    const result = await browserSendTelegram({ botToken: 'tok', chatId: '123', text: 'Hello' });
    expect(global.fetch).toHaveBeenCalledTimes(1);
    expect(result.ok).toBe(false);
  });

  it('does not retry on 403 Forbidden', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: false,
      status: 403,
      json: async () => ({ ok: false, description: 'Forbidden' })
    });
    const result = await browserSendTelegram({ botToken: 'tok', chatId: '123', text: 'Hello' });
    expect(global.fetch).toHaveBeenCalledTimes(1);
    expect(result.ok).toBe(false);
  });

  it('retries on 500 server error up to 3 attempts', async () => {
    let callCount = 0;
    global.fetch = vi.fn().mockImplementation(async () => {
      callCount++;
      return {
        ok: false,
        status: 500,
        json: async () => ({ ok: false, description: 'Internal Server Error' })
      };
    });
    vi.spyOn(console, 'warn').mockImplementation(() => {});
    vi.useFakeTimers();
    const promise = browserSendTelegram({ botToken: 'tok', chatId: '123', text: 'Hello' });
    await vi.advanceTimersByTimeAsync(7000);
    const result = await promise;
    expect(callCount).toBe(3);
    expect(result.ok).toBe(false);
    vi.useRealTimers();
  });

  it('returns error on network failure after max retries', async () => {
    let callCount = 0;
    global.fetch = vi.fn().mockImplementation(async () => {
      callCount++;
      throw new Error('fetch failed');
    });
    vi.spyOn(console, 'warn').mockImplementation(() => {});
    vi.useFakeTimers();
    const promise = browserSendTelegram({ botToken: 'tok', chatId: '123', text: 'Hello' });
    await vi.advanceTimersByTimeAsync(7000);
    const result = await promise;
    expect(callCount).toBe(3);
    expect(result.ok).toBe(false);
    expect(result.error).toContain('fetch failed');
    vi.useRealTimers();
  });

  it('sends correct JSON payload', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ ok: true, result: { message_id: 1 } })
    });
    await browserSendTelegram({ botToken: 'tok', chatId: '42', text: 'Test message' });
    const [url, opts] = global.fetch.mock.calls[0];
    expect(url).toBe('https://api.telegram.org/bottok/sendMessage');
    expect(opts.method).toBe('POST');
    const body = JSON.parse(opts.body);
    expect(body.chat_id).toBe('42');
    expect(body.text).toBe('Test message');
    expect(body.disable_web_page_preview).toBe(true);
  });
});

describe('parseTelegramCommand', () => {
  it('parses /start with no args', () => {
    const result = parseTelegramCommand('/start');
    expect(result.cmd).toBe('start');
    expect(result.args).toBe('');
  });

  it('parses /ask with args', () => {
    const result = parseTelegramCommand('/ask what is 2+2');
    expect(result.cmd).toBe('ask');
    expect(result.args).toBe('what is 2+2');
  });

  it('returns null for non-command text', () => {
    expect(parseTelegramCommand('hello')).toBeNull();
  });

  it('returns null for empty string', () => {
    expect(parseTelegramCommand('')).toBeNull();
  });

  it('returns null for null/undefined input', () => {
    expect(parseTelegramCommand(null)).toBeNull();
    expect(parseTelegramCommand(undefined)).toBeNull();
  });

  it('strips bot mention from command', () => {
    const result = parseTelegramCommand('/help@alphonsobot');
    expect(result.cmd).toBe('help');
  });

  it('lowercases command name', () => {
    const result = parseTelegramCommand('/START');
    expect(result.cmd).toBe('start');
  });

  it('preserves multi-word args', () => {
    const result = parseTelegramCommand('/ask sum the numbers 1 2 3');
    expect(result.args).toBe('sum the numbers 1 2 3');
  });
});

describe('handleTelegramBotCommand', () => {
  it('returns ok: false for non-command text', async () => {
    const result = await handleTelegramBotCommand({ text: 'not a command', chatId: '1', botToken: 'tok' });
    expect(result.ok).toBe(false);
    expect(result.error).toBe('not_a_command');
  });

  it('returns ok: false for unknown command', async () => {
    const result = await handleTelegramBotCommand({ text: '/unknown', chatId: '1', botToken: 'tok' });
    expect(result.ok).toBe(false);
    expect(result.error).toBe('unknown_command');
  });

  it('returns reply string for /start without sending', async () => {
    const result = await handleTelegramBotCommand({ text: '/start', chatId: null, botToken: null });
    expect(result.ok).toBe(true);
    expect(typeof result.reply).toBe('string');
    expect(result.reply).toContain('Alphonso');
  });

  it('returns reply string for /help when no botToken', async () => {
    const result = await handleTelegramBotCommand({ text: '/help', chatId: null, botToken: null });
    expect(result.ok).toBe(true);
    expect(typeof result.reply).toBe('string');
    expect(result.reply.length).toBeGreaterThan(0);
  });

  it('returns reply string for /status without sending', async () => {
    const result = await handleTelegramBotCommand({ text: '/status', chatId: null, botToken: null });
    expect(result.ok).toBe(true);
    expect(result.reply).toContain('Alphonso is online');
  });

  it('returns usage hint for /ask with no args', async () => {
    const result = await handleTelegramBotCommand({ text: '/ask', chatId: null, botToken: null });
    expect(result.ok).toBe(true);
    expect(result.reply).toContain('Usage: /ask');
  });

  it('returns preview for /ask with args', async () => {
    const result = await handleTelegramBotCommand({ text: '/ask summarize this', chatId: null, botToken: null });
    expect(result.ok).toBe(true);
    expect(result.reply).toContain('Routing to Jose');
    expect(result.reply).toContain('summarize this');
  });

  it('truncates long /ask args to 80 chars', async () => {
    const longQuestion = 'a'.repeat(100);
    const result = await handleTelegramBotCommand({ text: `/ask ${longQuestion}`, chatId: null, botToken: null });
    expect(result.reply).toContain('...');
  });
});

describe('verifyTelegramBotEnvironment', () => {
  it('returns ok: false when botToken is missing', async () => {
    const result = await verifyTelegramBotEnvironment({});
    expect(result.ok).toBe(false);
    expect(result.trust).toBe('failed');
    expect(result.error).toContain('not configured');
  });

  it('returns ok: false on fetch error', async () => {
    global.fetch = vi.fn().mockRejectedValue(new Error('DNS failure'));
    const result = await verifyTelegramBotEnvironment({ botToken: 'bad-token' });
    expect(result.ok).toBe(false);
    expect(result.trust).toBe('failed');
    expect(result.error).toContain('DNS failure');
  });

  it('returns ok: true with botUsername on success', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ ok: true, result: { username: 'alphonsobot' } })
    });
    const result = await verifyTelegramBotEnvironment({ botToken: 'valid-token' });
    expect(result.ok).toBe(true);
    expect(result.botUsername).toBe('alphonsobot');
    expect(result.trust).toBe('verified');
  });

  it('returns ok: false when Telegram API returns ok: false', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ ok: false, description: 'Unauthorized' })
    });
    const result = await verifyTelegramBotEnvironment({ botToken: 'bad-token' });
    expect(result.ok).toBe(false);
    expect(result.trust).toBe('failed');
    expect(result.error).toBe('Unauthorized');
  });

  it('returns ok: false on non-200 HTTP status', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: false,
      status: 404,
      json: async () => ({ ok: false, description: 'Not Found' })
    });
    const result = await verifyTelegramBotEnvironment({ botToken: 'bad-token' });
    expect(result.ok).toBe(false);
    expect(result.trust).toBe('failed');
  });

  it('calls Telegram getMe endpoint with correct token', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ ok: true, result: { username: 'bot' } })
    });
    await verifyTelegramBotEnvironment({ botToken: 'my-token' });
    expect(global.fetch).toHaveBeenCalledWith('https://api.telegram.org/botmy-token/getMe');
  });

  it('returns empty string botUsername when username is null', async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ ok: true, result: { username: null } })
    });
    const result = await verifyTelegramBotEnvironment({ botToken: 'tok' });
    expect(result.botUsername).toBe('');
  });
});
