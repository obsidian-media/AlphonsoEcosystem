...
import { SENTINEL_BASE_PACKS } from '../../services/skillPackContentSentinel';

describe('skillPackContentSentinel', () => {
  it('exports SENTINEL_BASE_PACKS as array', () => {
    expect(Array.isArray(SENTINEL_BASE_PACKS)).toBe(true);
  });
  it('has at least 5 packs', () => {
    expect(SENTINEL_BASE_PACKS.length).toBeGreaterThanOrEqual(5);
  });
  it('each pack has required fields', () => {
    for (const pack of SENTINEL_BASE_PACKS) {
