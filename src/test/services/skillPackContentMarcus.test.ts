...
import { MARCUS_BASE_PACKS } from '../../services/skillPackContentMarcus';

describe('skillPackContentMarcus', () => {
  it('exports MARCUS_BASE_PACKS as array', () => {
    expect(Array.isArray(MARCUS_BASE_PACKS)).toBe(true);
  });
  it('has at least 5 packs', () => {
    expect(MARCUS_BASE_PACKS.length).toBeGreaterThanOrEqual(5);
  });
  it('each pack has required fields', () => {
    for (const pack of MARCUS_BASE_PACKS) {
