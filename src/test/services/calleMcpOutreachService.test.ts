...
describe('storage layer', () => {
  ...
  it('persisting a record via handleMcpOutreachMessage makes it retrievable, keyed correctly, and updates the shared index', async () => {
    mockPlanCall.mockResolvedValueOnce({ plan_id: 'p1', ready_to_run: false, next_step: 'ask', clarifying_questions: ['What phone number?'], confirm_summary: '' });
  ...
  it('a second chat gets its own independent record without disturbing the first', async () => {
    mockPlanCall
      .mockResolvedValueOnce({ plan_id: 'pA', ready_to_run: false, next_step: 'ask', clarifying_questions: ['q'], display_goal: 'Call A goal', confirm_summary: '' })
      .mockResolvedValueOnce({ plan_id: 'pB', ready_to_run: false, next_step: 'ask', clarifying_questions: ['q'], display_goal: 'Call B goal', confirm_summary: '' });
    await handleMcpOutreachMessage('chat-1', 'call A');
    await handleMcpOutreachMessage('chat-2', 'call B');

    expect((await getMcpOutreachRecord('chat-1'))?.goal).toBe('Call A goal');
    expect((await getMcpOutreachRecord('chat-2'))?.goal).toBe('Call B goal');
    const index = JSON.parse(kvStore['calle_mcp_outreach_index']);
...
describe('clarifying-question state machine', () => {
  it('stays in clarifying across multiple rounds, threading plan_id (not a conversation_history array -- the real API has no such param)', async () => {
    mockPlanCall
      .mockResolvedValueOnce({ plan_id: 'p1', ready_to_run: false, next_step: 'ask', clarifying_questions: ['What phone number?'], confirm_summary: '' })
      .mockResolvedValueOnce({ plan_id: 'p1', ready_to_run: false, next_step: 'ask', clarifying_questions: ['What should I ask them?'], confirm_summary: '' });

    await handleMcpOutreachMessage('chat-1', 'call Joe\'s Pizza');
    await handleMcpOutreachMessage('chat-1', '+15550123456');

    expect(mockPlanCall).toHaveBeenNthCalledWith(1, { userInput: 'call Joe\'s Pizza' });
    expect(mockPlanCall).toHaveBeenNthCalledWith(2, { planId: 'p1', userInput: '+15550123456' });
  ...
  it('moves to ready_to_confirm once plan_call reports ready_to_run', async () => {
    mockPlanCall.mockResolvedValueOnce({
      plan_id: 'p1', ready_to_run: true, next_step: 'confirm', confirm_token: 't1',
      confirm_summary: 'Ready to call Joe\'s Pizza at +15550123456'
    });
    ...
    expect(record?.confirmSummary).toBe('Ready to call Joe\'s Pizza at +15550123456');
  });

  it('"cancel" during clarifying deletes the record entirely', async () => {
    mockPlanCall.mockResolvedValueOnce({ plan_id: 'p1', ready_to_run: false, next_step: 'ask', clarifying_questions: ['q'], confirm_summary: '' });
    await handleMcpOutreachMessage('chat-1', 'call Joe\'s Pizza');
  ...
  it('re-sending a call-like message while ready_to_confirm tells the user to use the buttons, not free text', async () => {
    mockPlanCall.mockResolvedValueOnce({ plan_id: 'p1', ready_to_run: true, next_step: 'confirm', confirm_token: 't1', confirm_summary: 's1' });
    await handleMcpOutreachMessage('chat-1', 'call Joe\'s Pizza');
...
describe('confirmMcpOutreachCall', () => {
  async function getToReadyToConfirm(chatId: string) {
    mockPlanCall.mockResolvedValueOnce({ plan_id: 'p1', ready_to_run: true, next_step: 'confirm', confirm_token: 't1', confirm_summary: 'Call Joe' });
  ...
  it('places the call, moves to in_progress, and starts polling', async () => {
    ...
    mockGetCallRun.mockResolvedValueOnce({ run_id: 'run1', status: 'COMPLETED', result: {} });
    const reply = await confirmMcpOutreachCall('chat-1');
  ...
  it('two concurrent confirms for the same chat result in exactly one runCall', async () => {
    ...
    mockGetCallRun.mockResolvedValue({ run_id: 'run1', status: 'COMPLETED' });
...
describe('cancelMcpOutreachCall', () => {
  it('deletes a pre-submission (ready_to_confirm) record', async () => {
    mockPlanCall.mockResolvedValueOnce({ plan_id: 'p1', ready_to_run: true, next_step: 'confirm', confirm_token: 't1', confirm_summary: 's' });
  ...
  it('does NOT delete an in_progress record -- CALL-E has no remote cancel, so dropping it here would silently lose tracking of a call still running', async () => {
    ...
    await handleMcpOutreachMessage('chat-1', 'call Joe');
    mockRunCall.mockResolvedValueOnce({ run_id: 'run1', status: 'QUEUED' });
    mockGetCallRun.mockResolvedValue({ run_id: 'run1', status: 'QUEUED' });
...
describe('terminal-result field mapping (result.extracted / result.post_summary / result.transcript)', () => {
  it('reads structuredResult/summary/transcript from the real result.* shape, not a nonexistent structuredContent field', async () => {
    mockPlanCall.mockResolvedValueOnce({ plan_id: 'p1', ready_to_run: true, next_step: 'confirm', confirm_token: 't1', confirm_summary: 's' });
    await handleMcpOutreachMessage('chat-1', 'call Joe');
    mockRunCall.mockResolvedValueOnce({ run_id: 'run1', status: 'QUEUED' });
    mockGetCallRun.mockResolvedValueOnce({
      run_id: 'run1',
      status: 'COMPLETED',
      result: {
        summary: 'short summary',
        post_summary: 'They are interested in a new website.',
        extracted: { interested_in_website: 'yes' },
    ...
    expect(record?.summary).toBe('They are interested in a new website.');
    expect(record?.structuredResult).toEqual({ interested_in_website: 'yes' });
    expect(record?.transcript).toBe('bot: hello\nuser: hi');
  });

  it('normalizes a status containing a literal space ("NO ANSWER") to match the underscore terminal set', async () => {
    mockPlanCall.mockResolvedValueOnce({ plan_id: 'p1', ready_to_run: true, next_step: 'confirm', confirm_token: 't1', confirm_summary: 's' });
    await handleMcpOutreachMessage('chat-1', 'call Joe');
    mockRunCall.mockResolvedValueOnce({ run_id: 'run1', status: 'QUEUED' });
    mockGetCallRun.mockResolvedValueOnce({ run_id: 'run1', status: 'NO ANSWER', result: {} });
    vi.useFakeTimers();
    await confirmMcpOutreachCall('chat-1');
    await vi.advanceTimersByTimeAsync(10_000);
    vi.useRealTimers();
