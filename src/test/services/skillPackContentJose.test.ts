...
import { JOSE_BASE_PACKS } from '../../services/skillPackContentJose';

describe('skillPackContentJose', () => {
  it('exports JOSE_BASE_PACKS as array', () => {
    expect(Array.isArray(JOSE_BASE_PACKS)).toBe(true);
  });
  it('has at least 5 packs', () => {
    expect(JOSE_BASE_PACKS.length).toBeGreaterThanOrEqual(5);
  });
  it('each pack has required fields', () => {
    for (const pack of JOSE_BASE_PACKS) {
