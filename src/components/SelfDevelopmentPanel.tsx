...
interface WorkspaceValidation {
  ...
  root: string;
  exists?: boolean;
  requiredEntries: { path: string; exists: boolean }[];
...
interface ProofHooks {
  writeStage: (stage: string, payload: Record<string, unknown>) => Promise<unknown>;
}

interface SelfDevelopmentPanelProps {
  settings: Settings;
  setSettings: React.Dispatch<React.SetStateAction<Settings>> | undefined;
  updateCheckState: { checkedAtMs?: number } | undefined;
  verificationLogs: unknown[];
  workspaceFoundation: Record<string, unknown>;
  nativeSelfDevProof: NativeSelfDevProof | null;
  setNativeSelfDevProof: React.Dispatch<React.SetStateAction<NativeSelfDevProof | null>> | undefined;
  nativeProofHooks: ProofHooks | null;
}
...
export function SelfDevelopmentPanel({
...
}: SelfDevelopmentPanelProps) {
  const [bundle, setBundle] = useState<PacketBundle | null>(() => getCurrentSelfDevelopmentPacketBundle());
  const [cycles, setCycles] = useState<PacketBundle[]>(() => listSelfDevelopmentCycles());
  ...
  const validateWorkspace = async (rootOverride: string = resolvedWorkspaceRoot) => {
    setValidating(true);
    try {
  ...
  const runCycle = async ({ proofMode = 'automated_native' } = {}) => {
    ...
    setError('');
    try {
      const next = await (runSelfDevelopmentCycle as any)({
  ...
  const runNativeProofCycle = async () => {
    ...
    setError('');
    try {
      const result = await (runNativeRc0Proof as any)({
  ...
  const packetSummary: PacketSummary = bundle?.packetSummary || {
  ...
  const auditSummary: AuditSummary = bundle?.auditSummary || { blockerCount: 0, partialCount: 0, needsSetupCount: 0, issueCount: 0, filesScanned: 0, todoCount: 0 };
  const followUpCount = Number((auditSummary as unknown as Record<string, unknown>)[followUpKey] || 0);
          ...
          <div className="space-y-2">
            ...
            <div className="rounded-2xl border border-white/10 bg-zinc-900/55 p-3 text-[11px] text-zinc-300">
              ...
              <div className="mt-2 grid grid-cols-1 gap-2 md:grid-cols-2">
                {(workspaceValidation?.requiredEntries || ['package.json', 'src', 'src-tauri', 'docs']).map((entry: string | { path: string; exists: boolean }) => {
...
function SummaryCard({ label, value, state = 'unknown', detail }: { label: string; value: React.ReactNode; state?: string; detail?: React.ReactNode }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-zinc-900/55 p-4">
      ...
      {detail !== undefined && <div className="mt-1 text-[11px] leading-relaxed text-zinc-500">{detail}</div>}
    </div>
  );
}
