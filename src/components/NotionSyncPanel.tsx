    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 text-[11px] text-[var(--text-3)]">
          {wired ? <Wifi className="w-3.5 h-3.5 text-[var(--success)]" /> : <WifiOff className="w-3.5 h-3.5 text-[var(--text-3)]" />}
          <span>
            {wired ? 'Notion co-source ready' : 'Notion not wired'}
            {wired && pullEnabled?.reason ? <span className="text-[var(--text-3)]"> — {pullEnabled.reason}</span> : null}
          </span>
        </div>
      ...
      {lastResult ? (
        <div className="rounded-lg bg-[var(--surface-2)] px-2.5 py-2 text-[11px] text-[var(--text-2)]">
          ...
          <pre className="text-[10px] whitespace-pre-wrap break-words text-[var(--text-2)]">
      ...
      <div>
        <div className="text-[10px] uppercase tracking-widest text-[var(--text-3)] mb-1">Recent records (max 10)</div>
        {recent.length === 0 ? (
                ...
                <li key={r.id || id} className="rounded-lg bg-[var(--surface-2)] px-2.5 py-1.5 flex items-center justify-between gap-2">
                  <div className="min-w-0">
                    <div className="text-[11px] text-[var(--text-2)] truncate">
                      ...
                      {corr.workflow_id ? <span className="text-[var(--text-3)]"> · {corr.workflow_id}</span> : null}
                    </div>
      ...
      <div>
        <button
          ...
          className="flex items-center gap-1.5 text-[10px] uppercase tracking-widest font-bold text-[var(--accent)] hover:text-[var(--accent-hover)]"
        >
          <FileText className="w-3.5 h-3.5" />
          {showReport ? 'Hide weekly report' : 'Show weekly report'}
          <Database className="w-3 h-3 text-[var(--text-4)] ml-1" />
