...
function hostFromUrl(webhookUrl) {
  try {
...
export function ToolConnectionsPanel() {
  const [connections, setConnections] = useState(() => listToolConnections());
  const [audit, setAudit] = useState(() => listToolConnectionAudit());
  ...
  const testConnection = async (connection = selectedConnection) => {
    if (!connection) {
  ...
  const liveProofConnection = async (connection = selectedConnection) => {
    if (!connection) {
      setNotice('Save or select a connection first.');
      return;
    }
