/**
 * Alphonso — Notion Sync Panel
 *
 * OperatorDashboard section that exposes the Notion co-source sync contract
 * (see services/notionSyncService.js). This is a read-mostly view: it surfaces
 * status, recent records, counters, and the weekly report. Manual pull/push
 * are exposed as explicit buttons so nothing happens in the background.
 *
 * No secrets are ever rendered. The Notion token is read indirectly via
 * `isNotionPullEnabled()` and `isNotionConnectorReady()`; if neither is true
 * the panel shows a "Notion not wired" state and the action buttons are
 * disabled.
 */
...
function MiniButton({ onClick, label, icon: Icon, disabled = false, kind = 'default' }) {
...
export function NotionSyncPanel() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [s, recs] = await Promise.all([
        getNotionSyncStatus({ limit: 250 }),
        listNotionSyncRecords({ limit: 250 })
      ]);
      setStatus(s);
      setRecords(Array.isArray(recs) ? recs : []);
    } catch (err) {
      setLastError(err?.message || 'Failed to load Notion sync status');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const report = useMemo(
    () => notionSyncWeeklyReport({ records, generatedAtMs: Date.now() }),
    [records]
  );
  ...
  const handlePull = useCallback(async () => {
    ...
    setLastResult(null);
    try {
      const databaseId = typeof import.meta !== 'undefined' && import.meta.env?.VITE_NOTION_DATABASE_ID
        ? import.meta.env.VITE_NOTION_DATABASE_ID
        : null;
      const result = await pullNotionDatabase({ databaseId, pageSize: 25 });
      setLastResult({ kind: 'pull', result });
      await refresh();
    } catch (err) {
      setLastError(err?.message || 'Pull failed');
    } finally {
      setBusyAction(null);
    }
  }, [refresh]);
  ...
  const handlePush = useCallback(async () => {
    ...
    setLastResult(null);
    try {
      const result = await pushAlphonsoTaskToNotion(SAMPLE_TASK);
      setLastResult({ kind: 'push', result });
      await refresh();
    } catch (err) {
      setLastError(err?.message || 'Push failed');
    } finally {
      setBusyAction(null);
    }
  }, [refresh]);
  ...
  const handleCopyReport = useCallback(async () => {
    try {
      ...
      setLastError(err?.message || 'Copy failed');
    }
  }, [report.markdown]);
