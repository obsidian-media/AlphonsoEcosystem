...
function resolveAssignment(item) {
  const packet = getPacketById(item.packetId);
...
export function ApprovalPanel({ pendingApprovals = [], commandId, onAllResolved }) {
  ...
  const handleApprove = (packetId) => {
    try {
      approvePacket(packetId, 'chatview-inline');
      setResolved((prev) => ({ ...prev, [packetId]: 'approved' }));
      setError(null);
    } catch (err) {
      setError(`Approve failed: ${String(err?.message || err)}`);
    }
  };

  const handleReject = (packetId) => {
    try {
