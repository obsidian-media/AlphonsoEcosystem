      ...
      <div className="space-y-2 text-xs">
        ...
        {rows.map(([agentId, permission]) => (
          <div key={agentId} className="rounded-lg p-2">
            ...
            <div className="text-[var(--text-3)]">cannot do: {permission.blocked.slice(0, 3).join(', ')}</div>
            <div className="text-[var(--text-3)]">needs approval: {permission.approvalRequired.slice(0, 3).join(', ')}</div>
            {agentProfiles?.[agentId]?.limitations?.length ? (
              <div className="mt-1 text-[var(--warning)]">limitations: {agentProfiles[agentId].limitations!.slice(0, 2).join(' | ')}</div>
            ) : null}
            <div className="text-[var(--info)]">unwired: external autonomy disabled by policy.</div>
          </div>
        ))}
      </div>
    </div>
