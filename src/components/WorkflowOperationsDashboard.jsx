...
export function WorkflowOperationsDashboard({ settings }) {
  const [workflows, setWorkflows] = useState(() => listWorkflowOperations());
  const [runs, setRuns] = useState(() => listWorkflowRuns());
  ...
  const receiptRows = useMemo(() => selectedRun ? listWorkflowReceipts({ workflowRunId: selectedRun.id }).slice(0, 30) : [], [selectedRun, runs]);
  const timelineRows = useMemo(() => selectedRun ? listWorkflowRunTimeline(selectedRun.id).slice(0, 40) : [], [selectedRun, runs]);
  const telemetryRows = useMemo(() => selectedWorkflow ? listWorkflowTelemetry({ workflowId: selectedWorkflow.id }).slice(0, 25) : [], [selectedWorkflow, runs]);
  const telemetrySummary = useMemo(() => summarizeWorkflowTelemetry(selectedWorkflow?.id || null), [selectedWorkflow, runs]);
  const workflowMemory = useMemo(
  ...
  const executeRun = () => {
    if (!selectedRun) return;
    executeWorkflowRun(selectedRun.id, { note: 'Executed from Workflow Operations Dashboard.' });
    refresh();
  };
          ...
          {selectedWorkflow && (
            <div className="rounded-xl border border-white/10 bg-zinc-900/55 p-3">
              ...
              <div className="space-y-2">
                {getAgentWorkflowParticipation(selectedWorkflow).map((item) => (
              ...
              <div className="mt-2 max-h-24 space-y-1 overflow-y-auto pr-1">
                {getAgentWorkflowParticipation(selectedWorkflow).map((item) => {
                  const contract = AGENT_EXECUTION_CONTRACTS[item.agent];
...
function displayWorkflowTruthState(status, run = null) {
...
function mapRunTone(status, run = null) {
...
function runRowShellClass(run, isSelected) {
...
function receiptRowShellClass(status, run = null) {
  const truth = displayWorkflowTruthState(status, run);
