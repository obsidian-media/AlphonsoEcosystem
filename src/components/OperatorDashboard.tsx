...
interface OperatorDashboardProps {
  ...
  setOperatorMode: (mode: boolean) => void;
  modes: Modes;
  ...
  onVerifyProcess: (processes: string[]) => void;
  onVerifyPaths: (paths: string[]) => void;
  onVerifyCommand: (program: string, args: string[]) => void;
  memoryItems: MemoryItem[];
  plugins: Plugin[];
  diskPluginManifests: DiskPluginManifest[];
  pluginAudit: PluginAuditEntry[];
  onTogglePlugin: (id: string, enabled: boolean) => void;
  onDiscoverPlugins: () => void;
  workspaceFoundation: WorkspaceFoundation;
  onToggleWorkspaceFeature: (key: string, enabled: boolean) => void;
  workspaceProof: WorkspaceProof | null;
  ocrCapability: OcrCapability | null;
  onRunWorkspaceProof: () => void;
  onCheckOcrCapability: () => void;
  workspaceSymbolIndex: WorkspaceSymbolIndex | null;
  onBuildSymbolIndex: () => void;
  onExecutePluginTool: (params: { manifestPath: string; pluginId: string; toolId: string; extraArgs: string[] }) => void;
  onValidatePluginManifest: (path: string) => void;
  lastPluginToolRun: PluginToolRunResult | null;
  lastManifestValidation: ManifestValidation | null;
  pluginSandboxPolicy: PluginSandboxPolicy | null;
  onUpdatePluginSandboxPolicy: (policy: Partial<PluginSandboxPolicy>) => void;
  auditChainProof: AuditChainProof | null;
  onRunOcrAdapter: (params: { adapter: string; imagePath: string | null; extraArgs: string[] }) => void;
  lastOcrAdapterRun: { adapter: string; exit_code: number } | null;
  snapshots: Snapshot[];
  onCreateSnapshot: () => void;
  onRestoreSnapshot: (id: string) => void;
  onBackupMemory: () => void;
  onRunRuntimeRepair: () => void;
  onRunReleasePreflight: () => void;
  onExportDiagnostics: () => void;
  coachMode: boolean;
  coachAlwaysOnTop: boolean;
  onToggleCoachMode: () => void;
  onToggleCoachTop: () => void;
  screenObserverState: ScreenObserverState | null;
  screenObserverLogs: ScreenObserverLog[];
  onRequestScreenObserverPermission: () => void;
  onStartScreenObserver: () => void;
  onStopScreenObserver: () => void;
  onUpdateScreenObserverSettings: (settings: { sampleEveryMs?: number; notificationsEnabled?: boolean; audioAlertEnabled?: boolean }) => void;
}
...
export function OperatorDashboard({
  ...
  const [program, setProgram] = useState<string>('ollama');
  const [args, setArgs] = useState<string>('list');
  const [pluginManifestPath, setPluginManifestPath] = useState<string>('');
  const [pluginId, setPluginId] = useState<string>('');
  const [pluginToolId, setPluginToolId] = useState<string>('');
  const [pluginExtraArgs, setPluginExtraArgs] = useState<string>('');
  const [ocrAdapter, setOcrAdapter] = useState<string>('version_check');
  const [ocrImagePath, setOcrImagePath] = useState<string>('');
  const [ocrExtraArgs, setOcrExtraArgs] = useState<string>('');
  const [sampleEveryInput, setSampleEveryInput] = useState<string>(String(screenObserverState?.sampleEveryMs || 5000));
  const [focusMode, setFocusMode] = useState<boolean>(() => localStorage.getItem('alphonso_operator_density_v1') !== 'full');
  const [openSections, setOpenSections] = useState<Set<string>>(() => new Set(['core', 'screen', 'verification']));
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      const result = unifiedWeeklyReport({
        eventsRecords: [],
        notionSyncRecords: await listNotionSyncRecords({ limit: 500 }),
