      ...
      <div className="mt-3 grid grid-cols-1 gap-3 lg:grid-cols-[1fr_auto]">
        <div className="rounded-xl border border-white/10 bg-zinc-900/55 p-3 text-[11px] text-zinc-300">
          ...
          <div className="mt-2 grid gap-1 md:grid-cols-2">
            ...
            <div>Path: <span className="text-zinc-500">{String(durableStatus?.path || 'not available')}</span></div>
          </div>
    ...
    <div className="rounded-xl border border-white/10 bg-zinc-900/55 p-3">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <div className="mt-1 text-[11px] text-zinc-500">{new Date(log.timestampMs as number).toLocaleString()} | {log.source || 'unknown source'}</div>
        </div>
    ...
    <div className="rounded-xl border border-white/10 bg-zinc-900/55 p-3">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-1 flex flex-wrap items-center gap-1.5 text-[11px] text-zinc-500">
            <AgentAvatar agentId={packet.fromAgent as string} name={packet.fromAgent as string} sizeClass="h-4 w-4" className="border-white/15" />
            <span>{packet.fromAgent as string}</span>
            <span>requests {packet.actionType || packet.packetType} for</span>
            <AgentAvatar agentId={packet.toAgent as string} name={packet.toAgent as string} sizeClass="h-4 w-4" className="border-white/15" />
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        {packet.status === 'pending_approval' && (
          <>
            <button onClick={() => onApprove(packet.id as string)} className="rounded-lg bg-emerald-500/20 px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest text-emerald-100">Approve</button>
            <button onClick={() => onReject(packet.id as string)} className="rounded-lg bg-red-500/20 px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest text-red-100">Reject</button>
          </>
        )}
...
function MemoryCard({ item }: { item: any }) {
  return (
    <div className="rounded-xl border border-white/10 bg-zinc-900/55 p-3">
      ...
      <div className="mt-2 text-[10px] text-zinc-600">{item.timestampMs ? new Date(item.timestampMs as number).toLocaleString() : 'No timestamp'}</div>
    </div>
  );
}
