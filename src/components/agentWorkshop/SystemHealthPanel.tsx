...
interface HealthSummary {
  buildStatus: { status: string; message: string; updatedAt: string | null };
  verificationStatus: string;
  pendingApprovals: number;
  failedAgentTasks: number;
  dependencyConflicts: number;
  memoryLoad: number;
  orchestrationQueue: number;
  agentActivity: number;
  openProposals: number;
}

export function SystemHealthPanel() {
  const [health, setHealth] = useState<HealthSummary>(() => getSystemHealthSummary());
