...
- `NVIDIA_TTS_CHATTERBOX_URL` and `NVIDIA_TTS_CHATTERBOX_DEFAULT_VOICE`.
...
Before production release, issue a real authenticated request with each TTS
model. Verify that the response contains valid WAV audio and play it on a real
iOS device. The Chatterbox endpoint and voice setting must be verified against
the enabled NVIDIA deployment before declaring that option complete.
