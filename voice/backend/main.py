...
@app.websocket("/ws")
async def ws_endpoint(ws: WebSocket, request: Request):
    # Validate the per-session token before accepting the upgrade. The token is
    # generated by voice_sidecar.rs and stored in the Tauri process; only the
    # desktop app that launched Voice OS knows it, so a web page on the same
    # machine cannot open this socket without the token.
