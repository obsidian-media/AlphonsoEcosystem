...
- [2026-09-10] **CALL-E CSP root cause found and fixed; new escalation-call service built; live verification of all of it still pending.** Session started from the 2026-09-07 entry below's three deferred live-verification steps. Investigating why "Connect via Browser Login" did nothing when clicked in the native app (branch `fix/calle-csp-allowlist`) found the actual root cause: `tauri.conf.json`'s CSP `connect-src` allowlist never included `api.heycall-e.com` (REST) or `seleven-mcp-sg.airudder.com` (MCP broker/server) -- both are called via plain frontend `fetch()`, and the webview silently blocks any unlisted domain with no visible error. This is very likely why no real call had ever been placed by either path -- not a credential or account issue, a CSP gap nobody had reason to check. Fixed, plus the same gap for 9 other connectors found in the same pass (DeepSeek, Perplexity, Tavily, NVIDIA NIM, Discord, Slack, GitHub, Composio, plugin marketplace). **Also built:** `escalationCallService.ts` -- watches `pending_approval` orchestration packets past a configurable threshold and places one approval-gated CALL-E call per packet, resolving it directly from the spoken approve/reject answer (see CLAUDE.md's Do Not Duplicate entry for full design). `tsc --noEmit` clean, lint clean, doc counts (`verify-doc-counts.mjs`) and Do Not Duplicate coverage (`verify-dnd-coverage.mjs`) both fixed and passing. **Not yet done, still needs a human at the keyboard plus a native (not `npm run dev`) build:** the three original deferred steps below (MCP login through Alphonso's own code, keychain survival, one real terminal `run_call`), *and* a first live test of the new escalation service end-to-end (a real packet stuck in `pending_approval`, a real escalation call placed, the packet actually resolving from the answer). This dev machine independently required disabling Windows Smart App Control and reducing `[profile.dev]` debug info (see `Cargo.toml`) to get a local build to even compile/link/run at all -- unrelated to the CALL-E work itself, noted here in case another session hits the same wall on this machine. Status: CSP fix done and pushed; escalation service built and committed, not yet pushed; all live-verification work still open.

- [2026-09-09] **PR #238 CI caught two real bugs this session's own local verification missed -- both fixed, resolved.** Pushing this branch's full history to CI for the first time (ahead of a real Windows/Linux release) surfaced two genuine issues neither local `npx playwright test` nor `npx vitest` runs had caught: **(1)** `e2e/a11y.spec.js`'s Boardroom test still had the exact `getByText('Mission Room', { exact: true })` strict-mode-violation locator (3 matching elements) that a throwaway verification script earlier this session had already found and worked around -- the fix was applied to the scratch script but never carried back into the real committed spec file. Fixed with `getByRole('heading', { name: 'Mission Room' }).first()`. **(2)** Once the locator was fixed and the real assertion could run, it surfaced a genuine, previously-undetected color-contrast failure: `MissionRoom.tsx`'s message timestamp used bare `opacity-45` on inherited `currentColor`, landing at ~4.1:1 against Mission Room's themed background (short of the 4.5:1 required for 10px bold text) -- missed by every earlier check in this session because those all exercised `AgentGrid.tsx`/Setup surfaces, never actually loaded a Boardroom message with real timestamp content through axe. First fix attempt (`--text-4`, the app's usual de-emphasis token) only reached 4.43:1 specifically on this page, short by a hair -- Mission Room layers a decorative `--accent-glow` blur under its `--surface-0` background, shifting the real composite just enough to matter, unlike every other surface `--text-4` was verified clean against. Used `--text-3` (one step less dim) instead of nudging `--text-4` globally, to avoid risking a regression on the surfaces already confirmed clean. Verified for real against a fresh `vite build` via `npx playwright test` (not dev-server HMR, given the stakes): full a11y suite 6/6 real tests passing plus the 1 already-known `test.fixme`, full E2E suite 34 passed/1 skipped/0 failed. `tsc --noEmit` and `npm run lint` both clean. Lesson: a fix applied to a throwaway verification script must be carried back into the real committed test, and pushing to real CI is itself a form of verification -- this session's own local test runs, however thorough, did not reproduce either bug. Status: closed, PR #238.

- [2026-09-09] **First real end-to-end `npm run tauri dev` walkthrough of the Smart Installer -- found and fixed one real live bug, resolved.** This was the actual first time anyone watched the built app run: past the Application Control block this dev machine has previously hit on freshly-compiled binaries (3m02s clean build, `target\debug\app.exe` ran without incident), through Boot Ritual Intro -> Intent Selection -> Recommended Setup (correctly showed `Ollama + starter model (llama3.2:3b)` as already installed, a real live confirmation of O2's bundling/detection work) -> the custom Agent Grid, then a clean shutdown (exit code 0). **Real bug found:** `AgentGrid.tsx`'s already-installed optional agents (Miya/Maria/Marcus on this machine) rendered an unchecked, still-clickable checkbox next to their "Already installed" label -- functionally inert either way (`toInstall` already filters by `installedNames` regardless of the checkbox), but visually implied the user still needed to opt in. Fixed to match Alphonso's own checked+disabled "nothing to do here" treatment: `isChecked` now also true when `alreadyInstalled`, checkbox `disabled` now also true in that case, and `selectedComponents` includes already-installed components without requiring a manual toggle. Fix hot-reloaded into the still-running session for live confirmation rather than requiring a restart. New regression test in `AgentGrid.test.jsx` (10/10 passing in that file); `tsc --noEmit` and targeted ESLint clean. **Boot Ritual Intro's animation was not conclusively confirmed on the first pass** -- the user saw the app boot but couldn't confirm the emblem/pulse/scanline rendered correctly in the ~2.6s window before it auto-advances. **Resolved same day with a deliberate second look:** relaunched `npm run tauri dev` (frontend-only changes since the last run, so the Rust binary didn't need rebuilding -- 44s vs. the first run's 3m02s), the user watched from process start this time, and confirmed all of it for real: clean emblem render, animated cyan glow/pulse/scanline sweep (not a static flash), readable HUD text ("ALPHONSO" / "INITIALIZING SYSTEM…" / "ESTABLISHING NEURAL LINK…"), and a correct auto-advance into System Scan with no click needed. Also got a second live screenshot of the Agent Grid fix from this same relaunch, confirming Miya/Maria/Marcus now render checked+disabled exactly as intended. **Benign, pre-existing, unrelated to Smart Installer:** Voice OS repeatedly failed to bind port 8766 in a retry loop throughout the session (something else already held it) -- not investigated further this pass, not a regression from any of today's work. Status: closed -- both the AgentGrid fix and the full boot ritual are now genuinely, visually confirmed working end-to-end.

- [2026-09-08] **`cargo test` mistaken for hung after ~13 minutes, stopped, then re-run and confirmed fine (154 passed, 0 failed, 1 ignored) — recorded as a process lesson, not a code issue.** While verifying the O2 boot-time-loading Rust command, `rustc`'s CPU time read identically flat across two checks a few minutes apart, which looked like a real hang and matched a documented risk on this exact machine (Application Control policy blocking freshly-compiled binary execution, `os error 4551`, per this file's 2026-09-07 smoke-test entry). Stopped it on that basis -- but this file's own 2026-07-28 entry already recorded that the ClickUp/YouTube `cargo test` runs took 45-55 minutes on this machine's constrained resources before completing cleanly, so a 13-minute observation window was nowhere near long enough to distinguish "hung" from "just slow." Re-ran it in the background without blocking further work on it, and it completed in full a while later with a clean result. Lesson: on this specific dev machine, a flat/quiet `cargo test` CPU reading over a few minutes is not evidence of a hang by itself -- check this file's own prior timing precedent before stopping a Rust test run early, and prefer restarting it in the background over abandoning verification when in doubt. Status: closed, no actual problem, process note only.

- [2026-09-08] **Full WCAG sweep across non-Setup surfaces — run for real, most findings fixed, one genuine finding still open.** Closes most of the "(1) full WCAG audit of the whole app" item the 2026-09-07/09-08 accessibility entries below tracked as deferred. Extended `e2e/a11y.spec.js` with a new describe block scanning Chat, Settings, Automation, AI Runtime Manager, and Boardroom against the full `wcag2a`/`wcag2aa`/`wcag21a`/`wcag21aa` axe-core ruleset (not just `color-contrast`, since these surfaces had never been scanned at all). **Methodology note, recorded because it wasted real time and could trip up a future session:** the first several runs against `npx playwright test` all failed with violations that turned out to be stale -- a `vite preview` server from earlier in this session was still bound to port 5173 (`reuseExistingServer: true` in `playwright.config.js` means Playwright skips its own build+serve step entirely when something already answers that URL), so every scan was hitting a 15-hour-old build that predated even the already-landed `--accent-contrast` token fix. Confirmed via `Get-CimInstance Win32_Process` showing the process's real start time, then cross-checked by doing a genuinely fresh `vite build` and diffing its output for the suspect class strings (absent) before trusting any further result. Per this file's own 2026-07-25 lesson ("never kill a process holding a shared port without asking first"), that stale process was left alone rather than killed -- verification instead ran against a disposable second build served on port 5199 via a throwaway Node+Playwright+axe script (not committed; CI always does a truly fresh build+bind, so this local artifact has no bearing on CI correctness). Real findings from the properly-fresh scan, and what happened to each:
  - **~16 more `text-white` (or equivalent) on `bg-[var(--accent)]` call sites**, beyond the ~20 the 2026-09-08 "Color-contrast re-verification" entry believed it had already caught: `ApprovalPanel.tsx`, `CompanionPairingPanel.tsx`, `EcosystemMaturityPanels.tsx`, `FilesView.tsx`, `GuidedTour.tsx`, `NotionSyncPanel.tsx`, `OllamaPreflightPanel.tsx`, `OperatorDashboard.tsx`, `SettingsView.tsx` (6 occurrences), `WorkflowBuilderView.tsx` (1 of 2 -- the other pairs `text-white` with `bg-[var(--accent-dim)]`, a translucent overlay token, deliberately left alone, see below). All fixed to `text-[var(--accent-contrast)]`, the exact same token the original fix introduced -- this was completing that fix's sweep, not inventing a new one. Fixed.
  - **Settings: 7 toggle switches missing `aria-checked`.** Not a JSX bug -- every switch already had `aria-checked={settings.X}` in source, but 7 of the underlying settings keys (`desktopMode`, `localOnlyMode`, `approvalMode`, `safeMode`, `privacyShieldActive`, `miyaCompanionPinned`, `joseCompanionPinned`) can be `undefined` in a fresh/mocked settings object, and React omits a boolean attribute entirely when its value is `undefined` rather than rendering `"false"`. Fixed by coercing with `!!` at each of the 7 JSX sites -- a real switch should never omit its checked state regardless of whether the settings object has hydrated that key yet.
  - **Settings: 6 unlabeled inputs** (Ollama API Endpoint, Workspace Root, OCR Engine Path, Updater Endpoint, Updater Public Key, Custom Target) -- each already had a visible `<label>` right above it, just never programmatically associated. Fixed with matching `id`/`htmlFor` pairs rather than adding redundant `aria-label`s, since a real visible label already existed.
  - **Boardroom: 2 unlabeled fields** (the Handoff generator's project-name input and objective textarea in `MissionRoom.tsx`, no visible label at all) -- fixed with `aria-label`.
  - **AI Runtime Manager: 8 nodes fail color-contrast against `--accent-muted`/`--agent-echo-glow`/`--agent-nova-glow` -- NOT fixed, left open on purpose.** These are alpha-transparent overlay tokens (12-30% opacity per `tokens.css`), unlike every other violation found this pass, which were solid opaque color pairs. The rendered contrast axe measured depends on whatever surface color the overlay composites over, which can differ by card placement and by theme -- a token bump here can't be verified correct the same deterministic way the solid-color fixes above could be; it needs per-usage-site, per-theme re-verification, which is realistically its own dedicated pass, not a blind numeric nudge tacked onto this one. The corresponding e2e test is committed as `test.fixme(...)` (not skipped silently, not asserted as passing) with the exact violation data and this reasoning inline, so it stays visible until someone actually does that verification. Status: open, resume hint is the fixme comment itself.
  Chat, Settings, Automation, and Boardroom are now real, enforced, permanent regression tests (0 violations, asserted, not `fixme`). AI Runtime Manager remains genuinely open. `tsc --noEmit` clean, targeted ESLint clean (`npm run lint` only covers `src/`, confirmed via `package.json`, so `e2e/`'s pre-existing unrelated `no-undef` warnings on Playwright's own ambient globals were not in scope), 70/70 targeted component tests still passing across all 11 touched files (no regressions from the class-string swaps). Status: 4/5 surfaces closed; AI Runtime Manager open and explicitly flagged, not faked as fixed.
...
- [2026-08-21] **Hermes Zero-Cost Mode bypass via a non-loopback endpoint**:
  ...
  Status: **FIXED 2026-09-16** — made the conservative choice explicitly
  flagged as a policy decision, not a technical default: only bare loopback
  (`localhost`/`127.0.0.1`/`::1`) counts as "local" for this exemption. A
  LAN address, a Tailscale IP, or any public host is now treated as
  non-local until a future pass deliberately widens it (the Tailscale case
  `docs/HERMES_AGENT_DELEGATION_PLAN.md` Phase 2 discusses is a real,
  legitimate future scenario — narrowing/widening this list later is a
  policy change, not a bug). New `isLoopbackHermesUrl(url)` helper in
  `hermesAgentConnector.ts`; `sendHermesAgentMessage` now checks it before
  the existing `evaluatePolicyGate` call — if Zero-Cost Mode is on, the
  call isn't pre-`approved`, and the saved endpoint isn't loopback, the
  call is blocked with the same `HermesBlocked` shape (`ok:false,
  blocked:true`) every other policy-gate rejection already uses, audited
  via `appendConnectorAudit('hermes_agents', 'send_blocked_non_loopback_endpoint', ...)`.
  4 new tests in `hermesAgentConnector.test.js` cover: blocked under
  Zero-Cost Mode with a non-loopback endpoint, allowed with `approved:true`,
  allowed when Zero-Cost Mode is off, and — the regression guard — a
  loopback endpoint still works exactly as before under Zero-Cost Mode
  (every pre-existing test in this file already uses `127.0.0.1` and
  needed no changes). `tsc --noEmit` clean.
...
- [2026-09-05] SSRF via redirect in `src-tauri/src/search.rs`'s `fetch_research_sources` (found by CodeRabbit review on PR #227, verified real and confirmed pre-existing on `main` before that PR): the reqwest client uses `.redirect(reqwest::redirect::Policy::limited(5))`, and `is_private_host(host)` is only checked once against the *initial* URL's host (line ~404) before `client.get(url).send()` is called. reqwest follows up to 5 redirects transparently with no re-check against the SSRF guard on each hop, so a malicious or compromised source URL could 302 to a private/internal address (e.g. cloud metadata `169.254.169.254`, localhost, an internal service) and the response body would be fetched and fed into Hector's snippet/synthesis pipeline anyway. Confirmed via `git show 4028dde:src-tauri/src/search.rs` that this exact code (redirect policy + single pre-fetch host check) already existed before PR #227, which only extracted the 420-char truncation into `build_snippet` — out of scope for that PR to fix. Resume hint: a real fix needs a custom `reqwest::redirect::Policy::custom(...)` closure that re-validates each redirect target's host before continuing — non-trivial because `is_private_host` is `async` (does DNS resolution) while reqwest's redirect policy callback is synchronous; likely needs either a blocking DNS resolve inside the closure, a pre-resolved allow/deny cache, or disabling automatic redirects entirely (`Policy::none()`) and manually following/validating each hop in a loop. Status: ~~open, not started.~~ **FIXED 2026-09-16** — took the third option from the resume hint: `fetch_research_sources`'s `reqwest::Client` now builds with `.redirect(Policy::none())`, and a new `fetch_with_ssrf_guard(client, start_url)` helper manually follows redirects up to the same 5-hop limit as before, re-validating every hop's host against `is_private_host` (async DNS resolution, same guard already used for the initial URL) before following it, and also rejecting a redirect to a non-`http`/`https` scheme or one missing a `Location` header outright (fail closed, not pass-through). `search_research_sources` (the other `Policy::limited(5)` call site) was left unchanged — it only ever requests a single hardcoded, trusted `html.duckduckgo.com` URL, not an attacker-influenceable one, so it was never the actual SSRF vector. 3 new `#[tokio::test]`s in `search.rs`'s test module spin up a real single-shot loopback HTTP server (`spawn_redirect_server`/`spawn_ok_server`, hand-rolled HTTP/1.1 responses over a raw `TcpListener` — no new mock-server crate dependency) and prove: a redirect to a private/link-local address is blocked, a redirect to a non-http(s) scheme is blocked, and a redirect response with no `Location` header fails closed. `cargo check` clean; the tests themselves could not be executed on this dev machine (`cargo test`'s build step hits the same pre-existing Windows Application Control block on freshly-compiled binaries documented elsewhere in this file, `os error 4551`) — CI is the first real run.
...
- [2026-09-07] **Accessibility work beyond the Setup-flow pass — explicitly deferred to a following pass by user decision.** The v1.1 accessibility pass scoped on 2026-09-07 covers only the Smart Installer Setup screens (reduced-motion handling for the JS-timer-driven activation sequence, `aria-live` announcements for async scan/install state, real `aria-label`s on the intent tiles, progress semantics on the install queue). Three related areas were considered and deliberately left out of that scope, to be taken on as the next pass rather than dropped: **(1) a full WCAG audit of the whole app** — the last real one was an axe-core sweep that found 635 contrast violations app-wide and was fixed at the token level (see the comments in `src/styles/tokens.css` around `--text-3`/`--text-4`), but that fix was never re-verified against a real browser, and no audit has covered the app's non-Setup surfaces since; **(2) keyboard-trap handling** — out of scope for Setup specifically because Setup has no modals or overlays to trap focus in, but the wider app does (`Modal.tsx`, `ApprovalModal.tsx`, `CoachHardInterruptOverlay.tsx`, `SentinelFindingModal.tsx`, `KeyboardShortcutsModal.tsx`) and none have been checked for focus trapping or restore-on-close; **(3) color-contrast changes** — the design tokens already had a documented contrast pass, so re-litigating colors inside the Setup pass would have duplicated it, but the same "never re-verified in a real browser" caveat applies. Note there is no axe-core or equivalent a11y tooling in `package.json` today, so a real audit means adding one (`@axe-core/playwright` would fit the existing Playwright E2E setup). **Status update (2026-09-08): (2) and (3) are now closed, (1) remains open.** Per the user's explicit `AskUserQuestion` selection on 2026-09-08 ("Keyboard-trap handling" and "Color-contrast re-verification," declining "WCAG audit (whole app)" and "macOS release pipeline" for that pass specifically), both were completed: **(2) keyboard-trap handling** — `src/hooks/useFocusTrap.ts` (new), wired into all 8 real modal/dialog surfaces (`ui/Modal.tsx`, `ApprovalModal.tsx`, `SentinelFindingModal.tsx`, `DigestPanel.tsx`, `MemorySearch.tsx`, `GuidedTour.tsx`, `KeyboardShortcutsModal.tsx`, `CoachHardInterruptOverlay.tsx`) — traps Tab/Shift+Tab, moves focus in on open without stealing focus a caller already set, restores focus to the pre-open element on close; deliberately does not touch Escape (each modal already owns that, `CoachHardInterruptOverlay.tsx` deliberately has none). **(3) color-contrast re-verification** — see the "Color-contrast re-verification against a real browser -- resolved" entry above/below this one for the full real-browser axe-core pass and the violations it found and fixed (a token bump from months earlier had never actually been re-verified, and wasn't fully sufficient once checked for real). **(1) the full WCAG audit of the whole app is explicitly NOT done — it was declined for this pass, not declined permanently, and remains a real, tracked gap.** To be unambiguous since "declined" and "deferred" read differently to a future reader: this is a deferral, the same status it has carried since 2026-09-07, not a decision that it isn't needed. It still needs: a systematic `@axe-core/playwright` sweep across every non-Setup surface (Chat, Settings, Boardroom, Runtime Hub, Mission Room, etc. — the axe-core tooling exists now, from the color-contrast work, so this is "run it everywhere else," not "add tooling from scratch"), plus a manual keyboard-only and screen-reader pass, since axe-core's automated checks cannot catch everything (e.g. it cannot verify that a focus-trapped modal's restore-on-close actually lands somewhere sensible, only that the trap exists). **Status update (2026-09-08, later same day): (1) is now mostly closed too.** See the "Full WCAG sweep across non-Setup surfaces" entry above for the real automated axe-core sweep across Chat/Settings/Automation/Runtime Manager/Boardroom and everything it found and fixed. 4 of 5 surfaces are now genuinely clean and permanently enforced; AI Runtime Manager has one real, explicitly-flagged remaining color-contrast issue (alpha-overlay tokens, needs its own verification pass, tracked as `test.fixme` not silently skipped). The manual keyboard-only and screen-reader pass this entry also called for was NOT done this pass — axe-core's automated checks were the whole scope here, and still can't catch everything a human pass would (e.g. whether tab order is sensible, whether screen-reader announcements read naturally). Status: (2) and (3) closed 2026-09-08; (1) mostly closed 2026-09-08 (automated sweep done, one real finding open, manual pass still not done).
...
- [2026-09-15] **Dependabot PR #160 (`react-dom` 18.3.1 → 19.2.8) is broken — bumps `react-dom` alone without a matching `react` bump.** Found while triaging a batch of open dependabot PRs for merge. `react` stayed pinned at `^18.3.1` in both `package.json` entries while only `react-dom` moved to `^19.2.8` — an invalid mismatched major-version pair. Confirmed live, not just inferred: CI's `Test & Build` job failed with 119 test files erroring `TypeError: Cannot read properties of undefined (reading 'S')` inside `node_modules/react-dom/cjs/react-dom-client.development.js` — the standard crash signature when react-dom's internal shared-secrets object doesn't match the loaded `react` version. Auto-merge disabled on the PR and a comment left explaining the exact failure. Resume hint: a real fix means bumping `react` to a matching `19.x` release alongside `react-dom` in the same PR — a deliberate React 19 migration with its own dedicated test pass (breaking changes in React 19 around refs/effects/StrictMode), not a routine dependency-bump merge. Status: open, PR left unmerged, auto-merge off.

- [2026-09-15] **RUSTSEC-2026-0285 (rustls `0.23.42` TLS 1.3 handshake-boundary flaw, advisory published 2026-09-14) was silently blocking `cargo audit --deny warnings` — and therefore the required `Rust Tests & Clippy` CI job — on every PR/push touching `src-tauri/Cargo.lock`, discovered only because PR #250's own (unrelated) CI run hit it.** Nothing in this repo's dependency tooling proactively flags a newly-published advisory against an already-committed `Cargo.lock` between CI runs — `cargo audit` only runs inside CI on a PR/push, so a lockfile that was clean yesterday can start failing today with zero code change, and the first person to notice is whoever's PR happens to run CI next. Fixed via `cargo update -p rustls --precise 0.23.45` (also bumps `rustls-webpki` 0.103.13→0.103.15) — applied on PR #250's branch directly and via a dedicated PR #251 for `main` (protected, no direct push), both merged same day. Resume hint: there's no scheduled/cron `cargo audit` job independent of PR activity — worth considering one (e.g. a daily `cargo audit` GitHub Action against `main`) so a fresh advisory surfaces via a dedicated alert instead of silently failing the next unlucky PR. Status: this specific advisory closed (rustls bumped on `main` and PR #250's branch); the "no proactive advisory scanning" gap itself is open, not started.
