...
8. **Get PR #151 to green on all required checks** (`Test & Build`, `Doc Count Freshness`, `Rust Tests & Clippy`, `Secrets Scan`, both `gate` checks) and hand it back for merge, or merge it yourself if you have that authority — check `AGENTS.md`'s repo-governance header first: branch-only workflow, no direct pushes to `main`, CI gate must be green to merge. This repo's `main` is protected; direct pushes will be rejected (confirmed empirically this session).
