...
- **Sidebar present** (the locked Arc/Notion-style sidebar from `draft-a-power-user-direction.md`'s Revision 2 "Rooms" skeleton — NOT the earlier, explicitly-rejected top-nav-with-dropdowns version). Home space's sidebar list gets 1 more real destination beyond the original Dashboard/Chat: **Session History** (`SessionHistoryView.tsx` — a real, existing component with no current obvious home before this). **Correction, made during `12-sidebar-redesign-plan.md`'s implementation:** Digest was NOT added as a sidebar item after all — direct verification of `App.tsx` at implementation time showed `DigestPanel` is a real, already-working overlay toggled by `digestOpen`/`setDigestOpen` state, triggered from a bell icon already wired in `TopBar.tsx`. Digest is an overlay, not a navigable page — it has no `activeTab` branch to link to, and adding a sidebar entry for it would either need to fake a nav item that doesn't switch tabs (inconsistent with every other sidebar item) or duplicate a trigger that already exists and works. Left out of the sidebar; Session History alone was wired (real `activeTab === 'session_history'` branch added to `App.tsx`, since it previously had zero navigation path anywhere — see `bug-log.md` #7).
...
- **Session History as a new Home-sidebar destination — CONFIRMED, done.** Digest is explicitly NOT a sidebar destination — see the corrected "Skeleton change" note above; it already has a real, working home via `TopBar.tsx`'s bell icon and was left there rather than duplicated.
