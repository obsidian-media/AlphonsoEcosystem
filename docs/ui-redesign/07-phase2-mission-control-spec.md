...
**Sentinel dropped from scope (found during plan-writing, not assumed during brainstorming):** verified `sentinelSecurityService.ts`'s only real caller — `RightPanel.tsx`'s `runQuickScan()` — calls `scanForThreats('', {})` with empty command text, which structurally can never match any threat pattern. There is currently no real Sentinel finding anywhere in this app to surface. Logged as Bug Log #4 (a real, pre-existing bug independent of this redesign) rather than building a source around it. **4 real sources, not 5**, throughout the rest of this document.
