...
Work search filters the currently loaded typed briefing on-device by run title, summary, owner, phase, trace identifier, or outcome detail. It does not make a new Cloud query or claim a complete server-side search result. Inbox applies the same local-only boundary to decision title, policy, affected resource, evidence, state, and summary fields while retaining the active-review, challenge-ready, and read-only recorded groupings.
