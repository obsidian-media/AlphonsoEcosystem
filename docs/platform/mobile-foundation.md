...
Work search filters the currently loaded typed briefing on-device by run title, summary, owner, phase, trace identifier, or outcome detail. Each meaningful search term may match a different field on the same record, so a combined owner-and-trace or policy-and-resource query remains useful. Search does not make a new Cloud query or claim a complete server-side search result. Inbox applies the same local-only boundary to decision title, policy, affected resource, evidence, state, and summary fields while retaining the active-review, challenge-ready, and read-only recorded groupings. A filtered Inbox renders only decision groups with matching records and uses one focused no-results state when none match. Both surfaces use the same labelled search control with a touch-safe clear action that restores the complete loaded briefing without requesting new Cloud data.
