...
Work search filters the currently loaded typed briefing on-device by run title, summary, owner, phase, trace identifier, or outcome detail. It does not make a new Cloud query or claim a complete server-side search result.
...
| Inbox decision queue | Typed `AtlasDecision` review, challenge-ready, and recorded states with an evidence-first detail handoff | `GET /v1/decisions` plus server-issued action challenge/receipt and durable terminal-decision history. |
