...
| `AlphonsoCompanionUITests/AlphonsoCompanionUITests.swift` | Smoke coverage for the Atlas default experience, workspace health, Home work initiation, Account & Cloud, audit trail, typed-direction preparation handoff, and the legacy return path. |
...
The mobile audit screen loads this receipt feed separately from the workspace briefing, reports unavailability without blocking core work, and intentionally renders an empty state in fixture-backed builds rather than inventing audit history.
...
| Work ledger & Library | Typed `AtlasRun` phase records plus a derived, read-only Work record and verified `AtlasOutcome` library records from the current briefing | `GET /v1/runs`, a correlated run event stream, and typed per-run detail/artifact and outcome-provenance contracts before richer evidence is claimed. |
