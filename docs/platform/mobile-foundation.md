...
The iOS application now has a new default product shell named **Atlas**. Atlas is the first implementation of Alphonso as a full mobile ecosystem client. It replaces the legacy companion-first navigation with **Home, Work, Inbox, Chat, and More** while the existing companion remains available as a reversible compatibility mode. Selecting a Work ledger entry—or an active-work record from Home—opens the same native **Work record** with the run’s purpose, owner, phase, verified update time, accountable next step, immutable trace, and—only when the authoritative briefing relates one—the decision/evidence checkpoint. The Work **Library** tab renders verified workspace outcomes and opens an outcome record with its result, completed time, and immutable trace; Home’s latest verified outcome links to that same record. These records are workspace evidence, not authorization or final external-action receipts.
Work search filters the currently loaded typed briefing on-device by run title, summary, owner, phase, trace identifier, or outcome detail. Each meaningful search term may match a different field on the same record, so a combined owner-and-trace or policy-and-resource query remains useful. Search does not make a new Cloud query or claim a complete server-side search result. Inbox applies the same local-only boundary to decision title, policy, affected resource, evidence, state, and summary fields while retaining the active-review, challenge-ready, and read-only recorded groupings. The audit trail likewise filters only the loaded read-only receipt event, decision, challenge, device, execution-status, and correlation fields; it does not claim a complete server-side audit search. A filtered Inbox renders only decision groups with matching records and uses one focused no-results state when none match. All three surfaces use the same visibly labelled search control rather than relying on placeholder-only cues, with a touch-safe clear action that restores the complete loaded record set without requesting new Cloud data.

The native Create work sheet holds its user in a clear preparation state, presents an explicit prepared-work receipt before leaving the sheet, and provides retryable local error feedback. Choosing **View prepared work** opens the Work tab’s **Planned** segment so the record is immediately visible. If the server accepts the draft but the local briefing is unavailable, Atlas preserves the prepared receipt and asks the user to refresh the authoritative workspace rather than retrying and risking a duplicate record.
Chat Studio accepts **typed direction** and carries it—or the current decision summary when no direction has been entered—into that same prefilled work draft. Voice capture, file intake, and generated suggestions remain unavailable until authenticated mobile contracts exist; they are not presented as active controls. A prepared record is a planned run only; it does not execute a task.
...
| `AlphonsoCompanionUITests/AlphonsoCompanionUITests.swift` | Smoke coverage for the Atlas default experience, workspace health, Home work initiation, local Work, Inbox, and audit filtering entry points, Work Library outcome records, Account & Cloud, audit trail, typed-direction preparation handoff, and the legacy return path. |
...
Inbox now groups every current decision record into **Needs your judgement**, **Confirmation queue**, and read-only **Recorded** sections. Review and challenge-ready records share the same evidence/policy detail route; an open review screen rebinds to the current authoritative decision state as the workspace updates. If a successful refresh no longer contains the focused decision, Atlas marks the local record unavailable and returns the user to the current Inbox rather than reusing it for review, challenge, or confirmation. Confirmation records open read-only accountability detail and remain explicitly non-executing evidence.

The client sends `Authorization: Bearer <access-token>`, `X-Alphonso-Device-Id`, `X-Alphonso-Client: ios`, and `X-Alphonso-API-Version: v1` on every workspace request. The Atlas identity bridge refreshes the existing user session, mirrors the short-lived access token into an Atlas-specific `ThisDeviceOnly` Keychain entry, and enrolls the matching device ID before the typed workspace store loads. After enrollment, the store opens the authenticated event feed and reconciles only complete server-authoritative briefings; it does not apply partial stream mutations. Home renders this transport state as a compact workspace-health record: an authoritative refresh snapshot, a live reconciled feed, an in-progress refresh, or a recoverable failure with an explicit refresh action. Refresh retrieves the briefing only; it does not execute work. Atlas starts review, challenge, and confirmation handoffs only from an actionable authoritative snapshot or live workspace state; failed, offline, idle, and in-progress sync states present an explicit refresh recovery instead. High-risk decisions then follow a separate sequence: record review, request the server challenge, complete Face ID locally, and submit a confirmation receipt. Atlas detects an expired actionable decision before recording review or requesting a challenge, then directs the operator to refresh the authoritative workspace; expired read-only accountability records remain available. Atlas also detects an expired challenge before starting local authentication or sending a confirmation request, then requires a fresh server challenge. If review, challenge, or confirmation cannot be recorded, the focused review screen displays an explicit recovery state instead of implying progress; the operator refreshes the authoritative workspace before retrying. If the review was recorded but challenge issuance fails, Atlas preserves that reviewed state and retries challenge issuance without attempting to record the review a second time. If confirmation recording fails after a biometric handoff, Atlas discards the local challenge and requires a fresh server challenge before another confirmation attempt.
The current demo receipt remains explicitly non-executing; production approval still requires a server-verifiable biometric proof and a distinct policy-gated execution adapter.
The mobile audit screen loads this receipt feed separately from the workspace briefing, reports the time of its last successful reconciliation, exposes a safe refresh action, and reports unavailability without blocking core work. Each loaded receipt shows its accountable decision reference when present and its correlation trace, but never exposes its raw device identifier. It intentionally renders an empty state in fixture-backed builds rather than inventing audit history.
...
| Workspace ribbon & health | `AtlasWorkspaceStore` via a factory that falls back to `AtlasFixtureRepository` when Cloud is unconfigured; typed snapshot/live/failed sync status with a refresh control | `WorkspaceSummary` plus health and execution-posture events, background recovery policy, and retained last-known-good snapshots. |
| Home | `AtlasBriefing` and correlated `AtlasRun`/`AtlasOutcome` records | `GET /v1/workspaces/{id}/briefing` and typed briefing events. |
| Work ledger & Library | Typed `AtlasRun` phase records plus a derived, read-only Work record and verified `AtlasOutcome` library records from the current briefing; local title/owner/phase/trace filtering | `GET /v1/runs`, a correlated run event stream, server-side search, and typed per-run detail/artifact and outcome-provenance contracts before richer evidence is claimed. |
...
| Inbox decision queue | Typed `AtlasDecision` review, challenge-ready, and recorded states with an evidence-first detail handoff and on-device title/policy/resource/evidence filtering of the loaded briefing | `GET /v1/decisions`, a server-side decision search contract, server-issued action challenge/receipt, and durable terminal-decision history. |
| Chat Studio | Typed workspace and decision context plus typed-direction handoff into native work preparation | Conversation/run API with structured event blocks, authenticated attachment transfer, speech capture, suggestion generation, and a durable conversational context model. |
