...
**Last verified:** 2026-08-12 — v2.6.1 (Mobile companion operations + ChatView race condition fix: added Task Launcher bento card, Agent Dock Interlock profiles sheet, and Boardroom live steering; resolved the chat-loading race bug; branch `fix/pr140-review-followup`)
**Verified by:** OpenCode session — designed and implemented premium "Midnight Editorial Cyberpunk" UI/UX styling for the iOS Companion using the `ultimate-frontend-design` framework and real cloud-hosted portrait assets; resolved a major latent race condition bug in `ChatView.tsx` by introducing synchronous message list clearing on activeChatId changes. Integrated remote image loading with AsyncImage, dual-storage updates in `approvalService.js` via `durableSet`, and JSON-RPC dispatch mappings in `companion_router.rs`. All local TypeScript typechecks and permission matrix tests are completely passing. Current repo totals: 263 test files, 3,758 tests, 185 services, 24 connectors.
