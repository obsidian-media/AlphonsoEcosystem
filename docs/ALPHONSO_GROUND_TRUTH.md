...
**Last verified:** 2026-07-28 — v2.6.1 (Connector/runtime + ComfyUI pass: verified the real ComfyUI Desktop install root, fixed launcher model-path wiring, proved a live `127.0.0.1:8188` image render, added focused ComfyUI regressions, refreshed repo summaries to current counts; branch `refactor/skillpack-jose-ts-migration`)
**Verified by:** Claude Code session — confirmed the real ComfyUI Desktop install root at `D:\Comfy-Desktop\ComfyUI-Installs\ComfyUI\ComfyUI` contains `main.py` and a bundled `.venv\Scripts\python.exe`; verified the installed Alphonso desktop profile still had `alphonso_settings.comfyuiDir` empty in WebView2 local storage; updated Alphonso so Settings auto-resolves both `comfyuiDir` and `comfyuiPython`, and so the Rust launcher writes a temporary `--extra-model-paths-config` for `D:\Comfy-Desktop\ComfyUI-Shared\models\...` before starting ComfyUI. Live proof: ComfyUI `0.20.1` on `http://127.0.0.1:8188` accepted prompt `32eeefec-624c-4fae-9efd-14f64a166e31`, completed successfully, and returned output `ALPHONSO_8188_SMOKE_00001_.png` using checkpoint `DreamShaper_8_pruned.safetensors`. Focused regressions passed: `src/test/services/comfyuiSettingsService.test.ts` (8/8) and `src/test/services/connectorImageGenerators.test.js` (22/22). Current repo totals as of this pass: 263 test files, 3,758 tests, 185 services, 24 connectors.
