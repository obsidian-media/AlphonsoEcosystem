...
### Production-readiness roadmap T3–T4, T11–T20 (not started, tracked 2026-07-15/16)
Full task list, plan, risk, and dependencies for each: `docs/PRODUCTION_READINESS_ASSESSMENT_2026-07-15.md` §6. T1/T2/T5/T6 are done and merged (see §11.21); T10 is partial (E2E collection fixed, specs still need repair). Do not re-derive scope for these from memory — read that doc's §6 before starting any of them, since each has explicit dependencies on the others (e.g. T4 branch protection explicitly depends on T1–T3, T13 keychain depends on T5's token format). Short list for quick reference:
- **T3** — triage the TruffleHog secret-scan finding + document a disclosure process
- **T4** — enable GitHub branch protection on `main` (blocked on T1–T3 being green first)
