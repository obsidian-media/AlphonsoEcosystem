...
**Last verified:** 2026-07-28 — v2.6.1 (Connector/runtime + ComfyUI pass: seeded the real ComfyUI Desktop install root, added runtime and connector smoke coverage, refreshed repo summaries to current counts; branch `refactor/skillpack-jose-ts-migration`)
**Verified by:** Claude Code session — confirmed the real ComfyUI Desktop install root at `D:\Comfy-Desktop\ComfyUI-Installs\ComfyUI\ComfyUI` contains `main.py` and `requirements.txt`, then taught Alphonso to auto-seed `settings.comfyuiDir` from the installed runtime path when the field is empty. Added new smoke coverage for previously weak runtime apps and connectors, and refreshed live summary docs to current counts. Current repo totals as of this pass: 263 test files, 3,795 tests, 173 services, 24 connectors.
