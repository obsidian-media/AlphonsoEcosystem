...
| **T13** OS-keychain credentials | ✅ done, 2026-07-25 + 2026-08-26 | connector creds via `os_keychain_store.rs` (B3, PR in `fix/truth-first-plan-abcd`); license tokens (PR #192) and Composio API keys (PR #193) migrated the same way 2026-08-26 — all 3 secret types in this repo are now off plaintext localStorage/SQLite |
| **T14** `lib.rs` split + `CREATE_NO_WINDOW` lint | 🟡 partial, updated 2026-09-02 | `lib.rs` cut from 2,206 → **837 lines** via PRs #194–#197 (2026-08-26/27); the CI-lint half (enforcing the shared `no_window()` helper on new spawns) is still not wired |
| **T15** live-verify signed updater | 🔒 `BLOCKED` | duplicate of Truth-First plan's D1 — needs owner go-ahead to cut a real public release; not attempted |
| **T16** live-verify iOS pairing | 🔒 `BLOCKED` | duplicate of Truth-First plan's D2 — needs physical device access; not attempted |
| **T17** cloud-sidecar observability | 🟡 partial, updated 2026-09-02 | gateways already had structured `safeLog()`; PR #203 (2026-09-02) extended it to `bridge/server.js` and `mcp-server/server.js`; a documented way to inspect these logs without host shell access is still missing |
| **T18** Supabase service-role-key exposure | 🟡 in progress, 2026-07-29 | Cloud Voice's `/auth` and `voice_devices` calls now use `SUPABASE_ANON_KEY` + the user's JWT under RLS instead of the raw service-role key (see Truth-First plan's F3); remains in-progress until the Railway env var is flipped and the RLS-backed flow is verified live against Supabase |
| **T19** auto-generate "Do Not Duplicate" map | 🟡 partial, updated 2026-09-02 | PR #201 wired an **advisory** CI checker (`verify:dnd-coverage`, `continue-on-error: true`) that flags gaps — it found **124 files** missing from CLAUDE.md's table on 2026-09-02. The generator itself (deriving the table from source) has not been built; the checker only detects drift, it doesn't fix it |
| **T20** multi-agent fan-out budget + surface hidden features | 🟡 partial, updated 2026-09-02 | PR #202 added an Operator Dashboard nav entry (closes one of the flagged low-discoverability surfaces); Agent Pairing and Ecosystem Maturity panels remain undiscoverable by the same standard, and no token/cost budget exists yet beyond the pre-existing `MAX_CHAIN_DEPTH=3` Boardroom hop cap |
