...
fn packet_from_finding(index: usize, finding: &WorkspaceReadinessFinding) -> NativeRc0Packet {
    ...
    NativeRc0Packet {
        ...
        expected_proof: "10_rc0_package_written.json plus refreshed RC0 evidence artifacts"
            .to_string(),
        rollback_note:
            "Revert the isolated patch and rerun proof:rc0 if the change regresses RC0 truth."
                .to_string(),
...
pub fn run_native_rc0_proof_engine(
    ...
    let proof_request =
        read_proof_request(input.output_dir.as_deref().unwrap_or(DEFAULT_OUTPUT_DIR));
