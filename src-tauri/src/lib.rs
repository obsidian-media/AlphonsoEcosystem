...
fn run_ocr_adapter(
    ...
    match adapter.as_str() {
        ...
        _ => {
            return Err(
                "Unsupported OCR adapter. Supported adapters: version_check, tesseract_cli."
                    .to_string(),
            );
        }
    }
...
fn decompose_jose_command_backend(command_text: String) -> Vec<JoseAssignmentProof> {
    ...
    if research {
        assignments.push(JoseAssignmentProof {
            ...
            rationale: "Research language detected. Hector should gather and verify sources."
                .to_string(),
    ...
    if publishing {
        assignments.push(JoseAssignmentProof {
            ...
            rationale:
                "Publishing language detected. Jose approval required before any external action."
                    .to_string(),
    ...
    if creative {
        assignments.push(JoseAssignmentProof {
            ...
            rationale:
                "Creative language detected. Miya produces script/storyboard/prompt packages."
                    .to_string(),
...
async fn check_app_update(
    ...
    if endpoint.is_empty() || pubkey.is_empty() {
        return AppUpdateCheckProof {
            ...
            trust: "unverified".to_string(),
            error: Some(
                "Updater is not configured. Provide both endpoint and public key.".to_string(),
            ),
        };
    }
