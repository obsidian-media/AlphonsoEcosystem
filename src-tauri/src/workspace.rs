    ...
    for path in target_paths {
            ...
            || files_scanned as usize >= max_files
        {
            return Ok(WorkspaceReadinessScan {
                ...
                error: Some(
                    "Readiness scan time budget reached before proof surface completed."
                        .to_string(),
                ),
            });
        }
        ...
        let _ =
            write_native_proof_stage(Path::new("release/rc0"), "08_scan_progress.json", &progress);
        ...
        if readiness_scan_timed_out(scan_started_at_ms) {
            return Ok(WorkspaceReadinessScan {
                ...
                trust: "partial".to_string(),
                error: Some(
                    "Readiness scan time budget reached after reading a proof surface file."
                        .to_string(),
                ),
            });
        }
        ...
        for (line_index, line) in content.lines().enumerate() {
            ...
            let message = if needle == "setup required" || needle == "setup-required" {
                "Setup-required surface should be labeled clearly and backed by real checks."
                    .to_string()
                ...
                "Production-facing surface should not imply fake or simulated execution."
                    .to_string()
            ...
            match kind {
                "placeholder" | "scaffold" | "demo" | "mock" | "not_wired" | "fake"
                | "simulated" => {
                    placeholder_count += 1;
                }
                "todo" => {
...
pub(crate) fn write_workspace_text_file(
    ...
    if !file_abs.starts_with(&root_abs) {
        return Err(
            "Path traversal detected: resolved path escapes workspace sandbox.".to_string(),
        );
    }
        ...
        for entry in fs::read_dir(dir).map_err(|e| e.to_string())? {
            ...
            if path.is_dir() {
                ...
                if let Some(ext) = path.extension().and_then(|e| e.to_str()) {
                    if ![
                        "js", "jsx", "ts", "tsx", "rs", "py", "go", "json", "md", "txt", "yml",
                        "yaml", "toml", "css", "html", "sh", "bat", "ps1",
                    ]
                    .contains(&ext)
                    {
                        continue;
    ...
    while let Some(dir) = stack.pop() {
        if readiness_scan_timed_out(scan_started_at_ms) {
            return Ok(WorkspaceReadinessScan {
                ...
                trust: "partial".to_string(),
                error: Some(
                    "Readiness scan time budget reached before traversal completed.".to_string(),
                ),
            });
        }
        ...
        for entry in entries.flatten() {
            if readiness_scan_timed_out(scan_started_at_ms) {
                return Ok(WorkspaceReadinessScan {
                    ...
                    trust: "partial".to_string(),
                    error: Some(
                        "Readiness scan time budget reached before traversal completed."
                            .to_string(),
                    ),
                });
            }
            ...
            if readiness_scan_timed_out(scan_started_at_ms) {
                return Ok(WorkspaceReadinessScan {
                    ...
                    trust: "partial".to_string(),
                    error: Some(
                        "Readiness scan time budget reached after reading a file batch."
                            .to_string(),
                    ),
                });
            }
            ...
            for (line_index, line) in content.lines().enumerate() {
                ...
                let message = if needle == "setup required" || needle == "setup-required" {
                    ...
                    "Updater or release path is blocked and should not imply completion."
                        .to_string()
                } else if kind == "connector_blocker" {
                    "Connector path is blocked and should be marked setup_required until real provider proof exists.".to_string()
                } else if kind == "not_wired" {
...
pub(crate) fn inspect_updater_release(
    ...
    if !bundle_path.exists() || !bundle_path.is_dir() {
        return Ok(ReleaseArtifactProof {
            ...
            trust: "failed".to_string(),
            error: Some(
                "Updater bundle directory does not exist or is not a directory.".to_string(),
            ),
        });
    }
