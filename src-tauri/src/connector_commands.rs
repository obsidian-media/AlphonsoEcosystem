...
pub(crate) async fn connector_send_notion(
    ...
    if parent.is_empty() {
        return Ok(ConnectorSendProof {
            ...
            trust: "unverified".to_string(),
            error: Some(
                "NOTION_PARENT_PAGE_ID is missing and no parent_page_id override was provided."
                    .to_string(),
            ),
        });
    }
...
pub(crate) async fn connector_send_clickup(
    ...
    if effective_list.is_empty() {
        return Ok(ConnectorSendProof {
            ...
            trust: "unverified".to_string(),
            error: Some(
                "CLICKUP_LIST_ID is missing and no list_id override was provided.".to_string(),
            ),
        });
    }
...
pub(crate) async fn tool_connection_post_webhook(
    ...
    if !is_allowed_webhook_url(&clean_url) {
        return Ok(WebhookPostProof {
            ...
            trust: "failed".to_string(),
            error: Some(
                "Webhook URLs must use https or localhost/http loopback for local testing."
                    .to_string(),
            ),
        });
    }
    ...
    match clean_id {
        "sd_webui" => {
            let endpoint = std::env::var("LOCAL_SDWEBUI_ENDPOINT")
                ...
                .trim_end_matches('/')
                .to_string();
            probe_local_runtime_health("sd_webui", "automatic1111", &endpoint, "/sdapi/v1/samplers")
                .await
        }
        "comfyui_video" => {
