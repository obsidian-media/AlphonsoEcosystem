...
pub async fn runway_generate_video(request: RunwayVideoRequest) -> RunwayVideoProof {
    ...
    let base_url = std::env::var("RUNWAYML_API_BASE_URL")
        .unwrap_or_else(|_| DEFAULT_RUNWAY_BASE_URL.to_string());
    let version = std::env::var("RUNWAYML_API_VERSION")
        .unwrap_or_else(|_| DEFAULT_RUNWAY_VERSION.to_string());
